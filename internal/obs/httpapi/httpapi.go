...
type API struct {
	...
	admission *obs.AdmissionService
	// persistPolicy is the opt-in write-through seam for the admission policy
...
// enrichment — handlers serve bare trajectory rows). admission may be nil
// (admission not wired — the /api/obs/admission/* routes report disabled and
// the trace detail carries no admission enrichment).
func New(store *obsstore.Store, enricher obs.ProxyEnricher, admission *obs.AdmissionService, logger *slog.Logger) *API {
	if logger == nil {
		logger = slog.Default()
	}
	return &API{store: store, enricher: enricher, admission: admission, logger: logger}
}
...
func (a *API) Routes() []Route {
	return []Route{
		...
		{"POST /api/obs/admission/policy", a.handleAdmissionSetPolicy},
...
type traceDetailResponse struct {
	...
	Admission   map[string][]obsstore.AdmissionEventView `json:"admission,omitempty"`
}
...
func (a *API) handleTrace(w http.ResponseWriter, r *http.Request) {
	...
	resp.Admission = a.admissionForSpans(r, detail.Spans)
	a.writeJSON(w, resp)
}

// admissionForSpans soft-joins admission verdicts to spans by request_id
// (admission spec §8 — a verdict renders as enrichment on the trajectory
// drawer). Returns nil when no admission events matched so the JSON field is
// omitted; the raw request is never present (only hashes + a gated reason).
func (a *API) admissionForSpans(r *http.Request, spans []obsstore.SpanRow) map[string][]obsstore.AdmissionEventView {
	if a.store == nil {
		return nil
	}
	var out map[string][]obsstore.AdmissionEventView
	for _, sp := range spans {
		if sp.RequestID == "" {
			continue
		}
		evs, err := a.store.AdmissionEventsForRequest(r.Context(), sp.RequestID)
		if err != nil {
			a.logger.Warn("obs httpapi: admission lookup failed", "request_id", sp.RequestID, "err", err)
			continue
		}
		if len(evs) == 0 {
			continue
		...
		if out == nil {
			out = make(map[string][]obsstore.AdmissionEventView)
		}
		out[sp.SpanID] = evs
	}
	return out
...
// writeJSONStatus is writeJSON with an explicit status code — headers are set
// BEFORE WriteHeader so a non-200 response still carries the JSON content type
// (e.g. the 422 lint-rejection body from the policy editor).
func (a *API) writeJSONStatus(w http.ResponseWriter, status int, v any) {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Cache-Control", "no-store")
