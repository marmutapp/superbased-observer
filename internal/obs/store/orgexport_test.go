...
func TestAggregateForOrg(t *testing.T) {
	...
	s := newDB(t)
	// Seed one day inside the 7-day aggregation window relative to now, so
	// the test cannot rot as the calendar advances (it previously hardcoded
	// 2026-06-29 and went red once that fell out of the window).
	...
	for i, r := range rows {
		byModel[r.Model] = i
		if r.Day != start.Format("2006-01-02") {
