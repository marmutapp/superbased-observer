...
// subdir. Linux + macOS are dir-based (grounded); Windows registers the
// manifest via the registry (HKCU\Software\<vendor>\NativeMessagingHosts),
// which this filesystem writer does not touch — the Windows rows are omitted
// so Detect/Register honestly report "no browser wired" there rather than
// writing a file the browser never reads.
