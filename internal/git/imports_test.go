...
// §24.1). internal/git is a pure, filesystem-only package (git root/branch/
// remote resolution via direct `.git` file reads, never shelling out to
// `git` and never touching the network); non-test source files must not
// reach for infrastructure imports (database/sql, net/http, fsnotify) or
// other observer subsystems (internal/store, internal/proxy, internal/db,
// internal/adapter, internal/watcher, internal/hook,
// internal/intelligence). Failing this test is a defect — the failure
// message names the file and the offending import so the offender is
// named at compile time.
//
// Tests may import broader stdlib infra without violating the boundary;
// only non-test files are scanned.
