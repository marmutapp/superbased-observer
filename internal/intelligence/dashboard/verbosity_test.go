...
func TestHandleSessionVerbosity_AuthoredNotCaptured(t *testing.T) {
	...
	if !resp.BackfillRecommended {
		t.Error("BackfillRecommended should be true when authored actions have no content_bytes")
	}
	if resp.AuthoredTotalActions == 0 || resp.AuthoredMeasuredActions != 0 {
		t.Errorf("authored action counts = measured:%d total:%d, want 0/non-zero", resp.AuthoredMeasuredActions, resp.AuthoredTotalActions)
	}
	if resp.BackfillSettingsURL != "/settings?section=backfill" {
		t.Errorf("BackfillSettingsURL = %q", resp.BackfillSettingsURL)
	}
}
