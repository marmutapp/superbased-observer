...
func (s *Server) handleSessionPredict(w http.ResponseWriter, r *http.Request, sessionID string) {
	...
	resp := PredictResponse{
		...
		// for this session's provider; falls back to needs-proxy when no
		// snapshot exists yet (hook-only / not routed through the proxy).
		Limit: loadLimitGauge(ctx, st, shape.Tool),
	}
...
// loadLimitGauge resolves the proxy-captured limit snapshot for the
// session's provider into the dashboard gauge shape. No snapshot →
// needs-proxy; a snapshot with subscription-window utilization →
// available; a snapshot without windows (classic-only) → no-window.
func loadLimitGauge(ctx context.Context, st *store.Store, tool string) LimitGauge {
	provider := providerForTool(tool)
	snap, ok, err := st.LatestLimitSnapshot(ctx, "default", provider)
	if err != nil || !ok {
		return LimitGauge{Available: false, NeedsProxy: true}
	}
