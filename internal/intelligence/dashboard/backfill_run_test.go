...
// TestBackfillRun_ScanModesDerivedFromRegistry pins the registry-derived
// per-adapter scan lane (the 2026-07-09 adapter-wave backfill decision:
// generic path, no per-tool backfill flags): every integration-registry
// adapter must have a `scan-<tool>` allowlist entry that execs the
// generic scanner scoped to that adapter, and the run handler must
// thread that argv (plus --config) into the job runner verbatim.
func TestBackfillRun_ScanModesDerivedFromRegistry(t *testing.T) {
	for _, c := range integration.Capabilities() {
		argv, ok := allowlistedBackfillModes["scan-"+c.Tool]
		if !ok {
			t.Errorf("adapter %q has no scan-%s allowlist entry — the init() derivation drifted", c.Tool, c.Tool)
			continue
		}
		want := []string{"scan", "--force", "--adapter", c.Tool}
		if len(argv) != len(want) {
			t.Errorf("scan-%s argv = %v, want %v", c.Tool, argv, want)
			continue
		}
		for i := range want {
			if argv[i] != want[i] {
	...
	// End-to-end through the run handler: the exec sees the scan argv,
	// not a backfill flag.
	fake := &fakeExecResult{out: []byte("rescan complete\n"), exit: 0}
	server := newServerWithFakeExec(t, newFakeExec(fake))
	rr := httptest.NewRecorder()
	server.Handler().ServeHTTP(rr,
		httptest.NewRequest(http.MethodPost, "/api/backfill/run", strings.NewReader(`{"mode":"scan-goose"}`)))
	if rr.Code != 200 {
		t.Fatalf("POST status: %d body=%s", rr.Code, rr.Body.String())
	}
	var runResp struct {
	...
	// Join on job completion before reading fake.sawArgs. The exec fn
	// writes sawArgs on the runBackfillJob goroutine, so busy-polling
	// the field directly is a data race. Polling the jobs endpoint
	// until the job leaves "running" instead establishes a
	// happens-before through the server's backfillMu: the exec fn
	// writes sawArgs and returns, then runBackfillJob sets the terminal
	// status under s.backfillMu — the same mutex this poll takes to
	// read it — so the subsequent sawArgs read is synchronized.
	deadline := time.Now().Add(2 * time.Second)
	for time.Now().Before(deadline) {
		jr := httptest.NewRecorder()
		server.Handler().ServeHTTP(jr,
	...
	got := strings.Join(fake.sawArgs, " ")
	if !strings.HasPrefix(got, "scan --force --adapter goose") || !strings.Contains(got, "--config") {
		t.Errorf("exec argv = %q, want scan --force --adapter goose … --config …", got)
	}
}
