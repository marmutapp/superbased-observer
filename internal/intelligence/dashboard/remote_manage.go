...
func (s *Server) handleRemoteEnable(w http.ResponseWriter, r *http.Request) {
	...
	info, err := remotecfg.Enable(cfg, cfgPath, remotecfg.EnableOptions{Host: host, AllowTerminal: body.AllowTerminal})
	if err != nil {
		writeErr(w, err)
		return
	}
...
func (s *Server) handleRemoteSessionRevoke(w http.ResponseWriter, r *http.Request) {
	...
	// that device (matched on the unified fingerprint = grant.Holder()), so a
	// device driving a PTY loses control the instant it is de-authorized (§8.1
	// item 8). The owner-local writer is never matched.
	if s.opts.LaunchManager != nil {
		s.opts.LaunchManager.RevokeRemoteWriterByHolder(fp, "device session revoked")
	}
...
func (s *Server) handleRemoteSessionsRevokeAll(w http.ResponseWriter, r *http.Request) {
	...
	// Terminate-all also kills every live remote writer lease (§8.1 item 8).
