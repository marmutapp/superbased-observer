...
// TestDetectCacheWriteWaste_HandoffRehydrationExempt proves the P3
// commit-17 exemption: the by-design cold rehydration write on a handoff
// target's first turn is excluded from the waste aggregation — both when
// the live lane attributed it as cause=handoff_rehydration AND, as a belt,
// when a non-proxied target's first event is only a plain reanchor but the
// session is stamped a handoff target.
func TestDetectCacheWriteWaste_HandoffRehydrationExempt(t *testing.T) {
	th := DefaultThresholds()
	// evs: a 500K leading write + a small, well-read tail. With the
	// leading write counted, read (63K) << 0.5×540K, so it fires; with
	// the leading write excluded, read (63K) ≥ 0.5×40K, so it doesn't.
	build := func(leadCause string, target bool) *Facts {
		s := SessionFacts{ID: "s-ho", Model: "claude-opus-4-7", Rows: mkRows(6, 1000, 100, 0, 0), HandoffTarget: target}
		f := testFacts(s)
		f.CacheEvents = map[string][]CacheEventFact{
			"s-ho": {
				{Kind: "reanchor", Cause: leadCause, TokensWritten: 500_000},
				{Kind: "write", Cause: "suffix_growth", TokensWritten: 20_000, TokensRead: 15_000},
				{Kind: "write", Cause: "suffix_growth", TokensWritten: 20_000, TokensRead: 18_000},
				{Kind: "hit", Cause: "suffix_growth", TokensRead: 30_000},
			},
		}
		return f
	}

	// Control: a plain reanchor lead on a NON-target session fires.
	if got := detectCacheWriteWaste(build("reanchor", false), th); len(got) != 1 {
		t.Fatalf("control: unmarked session with a big cold lead write should fire: got %+v", got)
	}
	// Cause-based exclusion: handoff_rehydration lead is excluded → no fire.
	if got := detectCacheWriteWaste(build("handoff_rehydration", false), th); len(got) != 0 {
		t.Fatalf("handoff_rehydration event must be excluded from waste: got %+v", got)
	}
	// Belt: plain reanchor lead but the session is a handoff target →
	// the leading event is excluded → no fire.
	if got := detectCacheWriteWaste(build("reanchor", true), th); len(got) != 0 {
		t.Fatalf("handoff-target first event must be excluded from waste: got %+v", got)
	}
}
