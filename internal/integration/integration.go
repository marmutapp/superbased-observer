...
var registry = map[string]Capability{
	...
	"qwen-code": {
		...
		//
		// ROUTE LANE — the config-lane writer is INSUFFICIENT on the live
		// install (grounded NEGATIVE 2026-07-09, operator-approved probe). The
		// OPENAI_BASE_URL env knob is inert, and rewriting model.baseUrl in
		// ~/.qwen/settings.json (proxyroute.RegisterQwenCode) does NOT route
		// either: this Qwen Code build carries a `modelProviders` map, and when
		// model.baseUrl does not match a configured provider it is IGNORED. The
		// probe rewrote model.baseUrl → http://127.0.0.1:8820/v1, but `qwen -p`
		// warned "Persisted model.baseUrl … no longer matches any provider for
		// model 'gpt-4o' (authType 'openai'); using the first id match
		// ('https://api.openai.com/v1')" and went DIRECT to api.openai.com,
		// bypassing the proxy — no api_turns row landed. modelProviders was
		// empty, so gpt-4o resolved via a built-in openai mapping. The .bak was
		// restored (config left as found). Routing this build would need the
		// writer to also point a matching modelProviders entry's baseUrl — out
		// of the current guarded writer's scope — so Proxy stays nil and
		// Routability STAYS probe_required. (Corrects the earlier note that
		// claimed model.baseUrl "pins the host".)
		Proxy: nil,
		ProxyProbe: &ProxyRoute{
			Kind:     RouteConfigFile,
			Launcher: "observer qwen",
			Note:     "proxyroute.RegisterQwenCode rewrites model.baseUrl in ~/.qwen/settings.json from the known default only; INSUFFICIENT on the live build (model.baseUrl is ignored when it matches no modelProviders entry — qwen bypassed to api.openai.com, no api_turns row, probe 2026-07-09); writer needs to also set a matching modelProviders baseUrl before this can route",
		},
		Routability: RouteStatusProbeRequired,
	...
	"devin": {
		...
		// A positional seed contract (`devin -- "<p>"`) exists in --help but
		// HANGS under a scripted pty — no launcher until the operator
		// verifies it on a real TTY.
		Handoff: HandoffCapability{
			Transcript: TranscriptFull,
			Inject:     []InjectKind{InjectFile},
			Note:       "positional seed contract unverified (hangs under scripted pty) — real-TTY verify owed before a launcher lands",
		},
	},
	"qoder": {
		Tool: "qoder",
