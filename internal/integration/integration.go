...
var registry = map[string]Capability{
	...
	"antigravity": {
		...
		// P0.1 SPLIT → PARTIAL: CLI (agy) plaintext .db readable (6/14
		// sessions on the Phase 0 corpus); desktop .pb OSCrypt-encrypted
		// (Windows cipher unknown) → actions_only there.
		Handoff: HandoffCapability{Transcript: TranscriptPartial, Inject: []InjectKind{InjectFile}, Note: "CLI .db readable; desktop .pb decrypt-gated"},
	},
}
