...
// apply mechanism on a machine whose config is not yet routed. qwen-code is
// the un-promoted case (Proxy nil / probe_required): its config-lane rewrite
// proved insufficient live (model.baseUrl is ignored when it matches no
// modelProviders entry), so it stays a "writer ready, promotion pending"
// probe.
func TestProxyProbeRouteKinds(t *testing.T) {
	want := map[string]RouteKind{
		...
		"qwen-code": RouteConfigFile,   // ~/.qwen/settings.json model.baseUrl (env lane inert; config lane insufficient on the modelProviders build)
	}
	// Only qwen-code remains an UN-promoted probe: Proxy must stay nil and
	// Routability probe_required. The promoted tools carry a verified Proxy.
	unpromoted := map[string]bool{"qwen-code": true}
	for tool, kind := range want {
		c, ok := For(tool)
