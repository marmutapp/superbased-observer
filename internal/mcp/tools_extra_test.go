...
// Smoke test: tools/list now returns 20 tools (12 spec §11.2 + the
...
// + continue_session, the session-handoff MCP lane —
// docs/session-handoff.md P2).
func TestServer_ToolsListReturnsTwenty(t *testing.T) {
	s, _, _ := testServer(t)
	resp := rpcCall(t, s, "tools/list", 1, nil)
	tools := resp["result"].(map[string]any)["tools"].([]any)
	if len(tools) != 20 {
		t.Errorf("tools count: %d (want 20)", len(tools))
	}
}
