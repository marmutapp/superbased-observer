...
// Smoke test: tools/list now returns 19 tools (12 spec §11.2 + the
...
func TestServer_ToolsListReturnsNineteen(t *testing.T) {
	s, _, _ := testServer(t)
	resp := rpcCall(t, s, "tools/list", 1, nil)
	tools := resp["result"].(map[string]any)["tools"].([]any)
	if len(tools) != 19 {
		t.Errorf("tools count: %d (want 19)", len(tools))
	}
}
