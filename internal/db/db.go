...
type Options struct {
	...
	// normally runs at the end of Open. Set this from short-lived
	// subprocesses that connect to the daemon-owned DB — the hook path
	// and the MCP server (`observer serve`): the daemon already runs the
	// probe at startup, so re-running it on every spawn just contends with
	// the daemon's WAL holder for no diagnostic benefit, and on a large DB
	// the quick_check can run for tens of seconds — long enough to push the
	// MCP server's first `initialize` response past the AI client's startup
	// timeout. Memory note `feedback_hook_db_open_no_timeout` captured the
	// contention; this option is the structural fix. Leave false for
	// daemons, tests, and `observer backfill` / `observer doctor` commands.
