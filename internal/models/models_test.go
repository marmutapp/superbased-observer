...
// TestIsMCPToolName pins the shared Claude-style MCP-name predicate that
// adapters use to promote mcp__<server>__<tool> calls to ActionMCPCall.
func TestIsMCPToolName(t *testing.T) {
	cases := []struct {
		...
		{"mcp__server", true}, // server only, no tool — still MCP
		{"Read", false},
		{"Bash", false},
		{"use_mcp_tool", false},      // Cline's naming — mapped explicitly, not here
		{"mcp_github_create", false}, // single underscore is a different convention
		{"", false},
	}
	for _, c := range cases {
		if got := IsMCPToolName(c.name); got != c.want {
			t.Errorf("IsMCPToolName(%q) = %v, want %v", c.name, got, c.want)
		}
	}
}
