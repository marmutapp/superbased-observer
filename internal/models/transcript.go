...
type TranscriptMessage struct {
	...
	Index int
	// ID is a stable per-message identifier lifted from the source
	// format's own record id (claude-code JSONL `uuid`) — the anchor a
	// message-addressable handoff surfaces so a target tool can re-request
	// the full message by id (get_session_message). It is honest-zero ("")
	// for formats that record no per-record id; nothing is fabricated. For
	// a merged assistant exchange it is the exchange's OPENING record's id
	// (first-wins), so the id points at a re-findable anchor in the source
	// file.
	ID string
