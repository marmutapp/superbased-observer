...
// topLevelModel extracts the request body's top-level "model" string WITHOUT a
// full parseRequest, reusing the topLevelValueSpan scanner. Returns "" when the
// field is absent, is not a JSON string, or the body does not parse (e.g. a
// gzip-encoded body the proxy did not decode — §3.5). Callers treat "" as
// "model unknown", so egress model matchers simply do not fire (fail-open).
//
// This resolves the requested model BEFORE admit() so the egress layer sees the
// pre-mutation top-level model (design §3.4), matching the body admission
// judged rather than any downstream Channel-B rewrite.
func topLevelModel(body []byte) string {
	...
	var m string
	if err := json.Unmarshal(spanValueBytes(body, span), &m); err != nil {
		return ""
	}
	return m
}
