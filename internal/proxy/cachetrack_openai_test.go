...
func TestBuildCacheObserveInput_OpenAI(t *testing.T) {
	...
	// This turn carries no cache_write_tokens (ChatGPT-plan lane), so the
	// pass-through leaves both cache_creation fields at 0 — byte-identical
	// to the pre-write-capture behaviour. The nonzero pass-through is
	// covered by TestBuildCacheObserveInput_OpenAI_CacheWritePassThrough.
	if in.Usage.CacheCreationTokens != 0 || in.Usage.CacheCreation1hTokens != 0 {
		t.Errorf("zero-write OpenAI turn must carry no cache_creation tokens; got creation=%d 1h=%d",
