...
func parseSSEStream(body []byte, provider string) streamResult {
	...
	case models.ProviderGoogle:
		return parseGeminiStream(body)
	default:
		return parseOpenAIStream(body)
	}
}

// parseGeminiStream extracts usage from a Google Gemini
// streamGenerateContent SSE stream (alt=sse): each `data:` line is a
// generateContent chunk, and usageMetadata is cumulative — the last chunk
// carrying it holds the final totals. Token mapping mirrors
// parseGeminiResponse (net input, reasoning folded into output).
func parseGeminiStream(body []byte) streamResult {
	...
	splitSSEEvents(body, func(lines [][]byte) {
		for _, line := range lines {
			...
			if raw.ModelVersion != "" {
				result.Model = raw.ModelVersion
			}
			if raw.ResponseID != "" {
				result.RequestID = raw.ResponseID
			}
			if len(raw.Candidates) > 0 && raw.Candidates[0].FinishReason != "" {
				result.StopReason = raw.Candidates[0].FinishReason
			}
			// usageMetadata is cumulative; overwrite with the latest non-zero
			// totals so the terminal chunk wins.
			if raw.UsageMetadata.PromptTokenCount > 0 || raw.UsageMetadata.CandidatesTokenCount > 0 {
				shape := raw.toShape()
				result.InputTokens = shape.InputTokens
				result.OutputTokens = shape.OutputTokens
				result.CacheReadTokens = shape.CacheReadTokens
			}
		}
	})
	return result
	...
	splitSSEEvents(body, func(lines [][]byte) {
		for _, line := range lines {
			...
			applyOpenAIUsage(
				...
				ev.Response.Usage.PromptTokensDetails.CacheWriteTokens,
				ev.Response.Usage.InputTokensDetails.CacheWriteTokens,
			)
			if ev.Model != "" {
			...
			applyOpenAIUsage(
				...
				ev.Usage.PromptTokensDetails.CacheWriteTokens,
				ev.Usage.InputTokensDetails.CacheWriteTokens,
			)
		}
	})
	return result
...
func applyOpenAIUsage(result *streamResult, promptTokens, completionTokens, inputTokens, outputTokens, promptCached, inputCached, promptWrite, inputWrite int64) {
	...
	// cache_write_tokens (Chat Completions: prompt_tokens_details;
	// Responses API: input_tokens_details) is billed at the 1.25×
	// cache-creation rate. It is a SUBSET of the non-cached input and
	// DISJOINT from cached_tokens — never additive, never subtracted
	// from input a second time (input-netting below uses reads only).
	// OpenAI writes are untiered, so the whole count goes in
	// CacheCreationTokens with CacheCreation1hTokens left at 0; the cost
	// engine then bills it exactly once at the base CacheCreation rate.
	// Always 0 on the ChatGPT-plan lane; live on a metered API-key turn.
	if promptWrite > 0 {
		result.CacheCreationTokens = promptWrite
	} else if inputWrite > 0 {
		result.CacheCreationTokens = inputWrite
	}
