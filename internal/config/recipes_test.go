...
func TestLoadRecipe_CodexSafe(t *testing.T) {
	...
	if !conv.DisableDrops {
		t.Errorf("codex-safe: disable_drops must be true (2026-07-16 drop-only degeneration fix)")
	}
...
// TestResolveCompression_CodexSafeDisableDrops pins that the no-drop
// knob actually LANDS on the codex lane through the profile-merge seam:
// the master config carries the back-compat default (disable_drops=false)
// and the codex-safe overlay flips it true, while master traffic keeps
// drops enabled. Guards the 2026-07-16 fix against a merge regression.
func TestResolveCompression_CodexSafeDisableDrops(t *testing.T) {
	t.Parallel()
	master := Default().Compression
	if master.Conversation.DisableDrops {
		t.Fatalf("master default disable_drops must be false (back-compat)")
	}
	resolved, err := ResolveCompression(master, "codex-safe")
	if err != nil {
		t.Fatalf("ResolveCompression: %v", err)
	}
	if !resolved.Conversation.DisableDrops {
		t.Errorf("codex-safe resolved disable_drops = false, want true (recipe overlay must win)")
	}
	// Master/default traffic must be unaffected — drops stay allowed.
	def, err := ResolveCompression(master, DefaultProfileName)
	if err != nil {
		t.Fatalf("ResolveCompression default: %v", err)
	}
	if def.Conversation.DisableDrops {
		t.Errorf("default profile disable_drops = true, want false (master passthrough)")
	}
}
