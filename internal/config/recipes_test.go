...
// compress_types=["json","logs","code"], stash disabled.
//
...
func TestLoadRecipe_ClaudeCode(t *testing.T) {
	...
	wantTypes := []string{"json", "logs", "code"}
...
// values: compress_types=["logs"], token mode, preserve_last_n=15,
...
func TestLoadRecipe_CodexSafe(t *testing.T) {
	...
	if !equalSlice(conv.CompressTypes, []string{"logs"}) {
		t.Errorf("codex-safe: compress_types = %v, want [logs]", conv.CompressTypes)
	}
...
// TestLoadAppliesRecipeAndValidates pins that Load(LoadOptions{
// RecipeName: ...}) yields a Config that validates and carries the
// recipe's distinctive values. This is the end-to-end path
// `observer start --recipe codex-safe` follows.
func TestLoadAppliesRecipeAndValidates(t *testing.T) {
	t.Parallel()
	cfg, err := Load(LoadOptions{
		RecipeName: "codex-safe",
		GlobalPath: filepath.Join(t.TempDir(), "absent.toml"),
		Env:        func(string) string { return "" },
	})
	if err != nil {
		t.Fatalf("Load with recipe: %v", err)
	}
	if !equalSlice(cfg.Compression.Conversation.CompressTypes, []string{"logs"}) {
		t.Errorf("Load did not apply codex-safe compress_types: got %v", cfg.Compression.Conversation.CompressTypes)
	}
}

// TestLoadRecipeOverridenByUserFile pins the merge precedence: a
// user's config.toml wins over the recipe. Operators on the
// codex-safe recipe who set their own preserve_last_n still get
// their value.
func TestLoadRecipeOverridenByUserFile(t *testing.T) {
	t.Parallel()
	cfgPath := filepath.Join(t.TempDir(), "observer.toml")
	body := []byte("[compression.conversation]\npreserve_last_n = 99\n")
	...
	if cfg.Compression.Conversation.PreserveLastN != 99 {
		t.Errorf("user override didn't win: got %d, want 99", cfg.Compression.Conversation.PreserveLastN)
	}
	// Recipe value still on the keys the user didn't override.
	if !equalSlice(cfg.Compression.Conversation.CompressTypes, []string{"logs"}) {
		t.Errorf("recipe compress_types should still apply: got %v", cfg.Compression.Conversation.CompressTypes)
	}
}
