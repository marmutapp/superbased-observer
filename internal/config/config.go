...
type BrowserConfig struct {
	...
	// turn is min(what the extension sent, this ceiling). Default
	// "usage_only" — the safest floor (no content is constructed unless the
	// operator raises the ceiling). An empty/unknown value is treated as
	// usage_only by the normalizer.
...
type TerminalConfig struct {
	...
	Enabled bool `toml:"enabled"`
	// MaxConcurrent caps live terminal sessions (default 4 — the termsession
	// default). 0 falls back to the termsession default.
...
func Default() Config {
	return Config{
		...
		// GranularityCeiling is the safest floor (usage_only — no content
		// stored unless raised). Same partial-merge rule as CacheTrack —
		// an install with no [browser] section keeps these values.
		// LOCAL-ONLY.
