...
func LoadRecipe(name string) (Config, error) {
	...
	path := "recipes/" + name + ".toml"
	body, err := recipesFS.ReadFile(path)
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			return Config{}, fmt.Errorf("config: unknown recipe %q (available: %s)", name, strings.Join(RecipeNames(), ", "))
		}
		return Config{}, fmt.Errorf("config: read recipe %q: %w", name, err)
	}
	cfg := Default()
