...
// singleCharTok maps punctuation runes that always lex to a fixed one-rune
// token. Multi-rune operators ('->', '<-', '<>') and literals are handled by
// the dedicated lexXxx helpers.
var singleCharTok = map[rune]tokKind{
	'(': tokLParen, ')': tokRParen,
...
func lex(s string) ([]token, error) {
	...
	for i < len(runes) {
		c := runes[i]
		if k, ok := singleCharTok[c]; ok {
			toks = append(toks, token{k, string(c)})
			i++
			continue
		}
		switch {
		case c == ' ' || c == '\t' || c == '\n' || c == '\r':
			i++
		case c == '-':
			tok, n := lexDash(runes, i)
			toks = append(toks, tok)
			i += n
		case c == '<':
			tok, n, err := lexAngle(runes, i)
			if err != nil {
				return nil, err
			}
			toks = append(toks, tok)
...
// lexDash lexes '->' (arrow) or a bare '-'. Returns the token and runes consumed.
func lexDash(runes []rune, i int) (token, int) {
	if i+1 < len(runes) && runes[i+1] == '>' {
		return token{tokRArrow, "->"}, 2
	}
	return token{tokDash, "-"}, 1
}

// lexAngle lexes '<-' (left arrow) or '<>' (not-equal). A lone '<' is an error.
func lexAngle(runes []rune, i int) (token, int, error) {
	if i+1 < len(runes) {
		switch runes[i+1] {
		case '-':
			return token{tokLArrow, "<-"}, 2, nil
		case '>':
			return token{tokNeq, "<>"}, 2, nil
		}
	}
	return token{}, 0, fmt.Errorf("query: unexpected '<' at %d", i)
}

// lexString lexes a single- or double-quoted string with backslash escapes,
// starting at the opening quote. Returns the token and runes consumed (including
// both quotes).
func lexString(runes []rune, i int) (token, int, error) {
	...
	if j >= len(runes) {
		return token{}, 0, fmt.Errorf("query: unterminated string starting at %d", start)
	}
	j++ // closing quote
	return token{tokString, sb.String()}, j - i, nil
}

// lexNumber lexes a run of ASCII digits. Returns the token and runes consumed.
func lexNumber(runes []rune, i int) (token, int) {
	j := i
	for j < len(runes) && runes[j] >= '0' && runes[j] <= '9' {
		j++
	}
	return token{tokNumber, string(runes[i:j])}, j - i
}

// lexIdent lexes an identifier (isIdentStart followed by isIdentPart runes).
// Returns the token and runes consumed.
func lexIdent(runes []rune, i int) (token, int) {
	j := i
	for j < len(runes) && isIdentPart(runes[j]) {
		j++
	}
	return token{tokIdent, string(runes[i:j])}, j - i
}
