...
func (s *Store) composeObsTiers(ctx context.Context, batch *PushBatch, est *int64, orgID, userEmail string, share ShareOptions) error {
	...
	if share.ObsAdmission && p.Admission != nil {
		ab, err := p.Admission(ctx, since, obsOrgRowCap)
		if err != nil {
	...
	if share.ObsEvalItems && p.EvalItems != nil {
		eb, err := p.EvalItems(ctx, since, obsOrgRowCap)
		if err != nil {
