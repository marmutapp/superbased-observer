...
func (s *Store) loadRunCommandActions(ctx context.Context, sessionID string) ([]processobs.ActionRef, error) {
	const q = `
		SELECT id, turn_index, COALESCE(target, ''), timestamp
		FROM actions
	...
	for rows.Next() {
		...
		var ts string
		if err := rows.Scan(&a.ActionID, &turn, &a.Command, &ts); err != nil {
...
// split, per-tool) for the status surface.
func (s *Store) ProcessStats(ctx context.Context) (ProcessStats, error) {
	st := ProcessStats{ByTool: map[string]int64{}}
	if err := s.db.QueryRowContext(ctx,
		`SELECT COUNT(*), COALESCE(SUM(session_id IS NOT NULL), 0) FROM process_runs`).
		Scan(&st.Total, &st.Attributed); err != nil {
