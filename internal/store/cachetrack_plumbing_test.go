...
// TestObservationToObserveInput_HandoffMarker proves the Tier-2 store
// boundary scans the reconstructed block bodies for the session-handoff
// marker so cachetrack attributes a handoff target's first-turn cold write
// as handoff_rehydration. observationToObserveInput is pure translation
// (reads only its argument), so a zero Store suffices.
func TestObservationToObserveInput_HandoffMarker(t *testing.T) {
	t.Parallel()
	s := &Store{}

	withMarker := models.CacheTurnObservation{
		SessionID: "sA", MessageID: "msg_a", Model: "claude-opus-4-7", Timestamp: t0Cache,
		BlockHashes: []models.CacheBlockMeta{
			{LevelLabel: "message", Kind: "text", CanonicalBytes: []byte(`{"text":"noise"}`), Role: "assistant"},
			{LevelLabel: "message", Kind: "text", CanonicalBytes: []byte(`{"text":"<!-- superbased-handoff abcd1234 -->"}`), Role: "user"},
		},
	}
	if in, _ := s.observationToObserveInput(withMarker); !in.HandoffMarker {
		t.Error("HandoffMarker should be true when a block body carries the marker")
	}

	noMarker := models.CacheTurnObservation{
		SessionID: "sA", MessageID: "msg_b", Model: "claude-opus-4-7", Timestamp: t0Cache,
		BlockHashes: []models.CacheBlockMeta{
			{LevelLabel: "message", Kind: "text", CanonicalBytes: []byte(`{"text":"plain prompt"}`), Role: "user"},
		},
	}
	if in, _ := s.observationToObserveInput(noMarker); in.HandoffMarker {
		t.Error("HandoffMarker should be false without the marker")
	}
}
