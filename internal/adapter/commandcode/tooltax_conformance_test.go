...
// rewrite".
//
// The private classifier here is the package-private actionMap keyed by normalizeToolKey (records.go).
//
...
func TestPrivateMappingAgreesWithTooltax(t *testing.T) {
	for _, tool := range []string{models.ToolCommandCode} {
		checked := 0
...
func TestPrivateActionMapIsSubsetOfTooltax(t *testing.T) {
	...
	for native, want := range actionMap {
		got, ok := tooltax.ResolveActionType(models.ToolCommandCode, native)
		if !ok {
			t.Errorf("tooltax has no row for %s %q (the adapter maps it to %q) — "+
				"add the name to internal/tooltax's table", models.ToolCommandCode, native, want)
			continue
		}
		if got != want {
			t.Errorf("drift: %s %q — adapter map says %q, tooltax says %q",
				models.ToolCommandCode, native, want, got)
		}
	}
}
