...
func TestParseCLIDB(t *testing.T) {
	...
	// The CLI-named adapter (defaults.go registers NewCLI() alongside New())
	// is the one that claims the agy-CLI .db layout; a desktop adapter
	// deliberately declines it (IsSessionFile name-gates on layout).
	a := NewWithOptions(nil, root).WithName(models.ToolAntigravityCLI)
	...
	// The CLI adapter must surface rows under antigravity-cli — the parse
	// helpers hardcode ToolAntigravity, so the ParseSessionFile wrapper
	// re-tags to a.Name(). Without it every CLI session is mislabeled
	// "antigravity" and blends into the desktop tool on the dashboard.
	for i, ev := range res.TokenEvents {
		if ev.Tool != models.ToolAntigravityCLI {
			t.Errorf("TokenEvents[%d].Tool = %q, want %q (NewCLI retag)", i, ev.Tool, models.ToolAntigravityCLI)
		}
	}
	for i, ev := range res.ToolEvents {
		if ev.Tool != models.ToolAntigravityCLI {
			t.Errorf("ToolEvents[%d].Tool = %q, want %q (NewCLI retag)", i, ev.Tool, models.ToolAntigravityCLI)
		}
	}
}
...
func TestParseCLIDB_RecoversTextAndProjectRoot(t *testing.T) {
	...
	// This adapter is desktop-named (NewWithOptions defaults to
	// ToolAntigravity), so the retag wrapper is a no-op — rows stay
	// "antigravity". Guards the wrapper against over-tagging.
	if got := res.TokenEvents[0].Tool; got != models.ToolAntigravity {
		t.Fatalf("desktop-named adapter tool = %q, want %q", got, models.ToolAntigravity)
	}
}
