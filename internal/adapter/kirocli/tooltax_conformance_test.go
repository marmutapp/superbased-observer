...
// re-implementation.
//
// The classifier is normalizeTool (normalize.go), an exact-name switch.
func classifyForTooltax(native string) string {
	return func() string { a, _, _ := normalizeTool(native, nil); return a }()
}

// tooltaxTools lists the tool ids this adapter's classifier feeds.
var tooltaxTools = []string{models.ToolKiroCLI}
...
var unclassifiedDomain = map[string]string{
	// DELIBERATE: `introspect` is Kiro asking about its own capabilities.
	// tooltax mirrors it as an explicit ActionUnknown row.
	"introspect": "deliberate: self-capability query, not agent work",
}
...
func TestClassifierDomainIsSubsetOfTooltax(t *testing.T) {
	domain, err := conformast.SwitchCaseStrings(".", "normalizeTool")
	if err != nil {
