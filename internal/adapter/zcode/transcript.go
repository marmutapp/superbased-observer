...
func (a *Adapter) ReadTranscript(ctx context.Context, sess models.Session, sourceHints []string) ([]models.TranscriptMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("zcode.ReadTranscript: open: %w", err)
	}
	defer db.Close()
...
// stateDBPath resolves db.sqlite: a usable hint first, else the first
// watch root holding one (D-P0.1). The live store's basename is
// "db.sqlite" (confirmed against a real ~/.zcode/cli/db/db.sqlite on
// both native and foreign-mount Windows installs) — NOT "zcode.db".
func (a *Adapter) stateDBPath(hints []string) (string, error) {
	for _, h := range hints {
		if strings.EqualFold(filepath.Base(h), "db.sqlite") && fileExists(h) {
			return h, nil
		}
	}
	for _, root := range a.roots {
		p := filepath.Join(root, "db.sqlite")
		if fileExists(p) {
			return p, nil
		}
	}
	return "", fmt.Errorf("zcode.ReadTranscript: no db.sqlite under any watch root")
}
...
func readSessionTranscript(ctx context.Context, db *sql.DB, sessionID string) ([]models.TranscriptMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("zcode.ReadTranscript: query: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		ts := zcodeMillis(msg.Time.Created)
		if msg.Role == "assistant" && zcodeMillis(msg.Time.Completed) != (time.Time{}) {
			ts = zcodeMillis(msg.Time.Completed)
		}
		...
		case msg.Role == "assistant" && head.Type == "text":
			...
			b.AssistantText(tp.Text, zcodeModel(msg), ts)
		case msg.Role == "assistant" && head.Type == "tool":
			var tp toolPartData
			if json.Unmarshal([]byte(partRaw), &tp) != nil {
				continue
			}
			b.AssistantCall(tp.CallID, tp.Tool, string(tp.State.Input), zcodeModel(msg), ts)
...
func zcodeModel(m messageData) string {
	if m.Model.ModelID != "" {
...
func zcodeMillis(ms int64) time.Time {
	if ms <= 0 {
		return time.Time{}
	}
