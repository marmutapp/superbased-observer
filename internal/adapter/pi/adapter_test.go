...
// TestParseSessionFile_IncrementalReparseKeepsSessionAndRoot reproduces the
// duplicate-session bug: an appended turn re-parsed from a non-zero offset
// must stay under the canonical (path-derived UUID) session id and the real
// cwd project root — NOT the filename basename + "[pi]" placeholder the
// scanner would otherwise fall back to after seeking past the `session` line.
func TestParseSessionFile_IncrementalReparseKeepsSessionAndRoot(t *testing.T) {
	root := t.TempDir()
	const uuid = "019f05c4-10dc-72de-8382-4991c000af2d"
	// Pi's real filename shape: "<timestamp>_<uuid>.jsonl".
	path := filepath.Join(root, "2026-06-26T21-09-23-295Z_"+uuid+".jsonl")

	header := strings.Join([]string{
		`{"type":"session","version":3,"id":"` + uuid + `","timestamp":"2026-06-26T21:09:23.295Z","cwd":"/home/me/proj"}`,
	...
	if got := first.ToolEvents[0].SessionID; got != uuid {
		t.Fatalf("first parse session id = %q, want %q", got, uuid)
	}

	// Append a second turn and re-parse from the saved cursor — this is the
	// path that previously stranded the turn under a second session.
	...
	for _, ev := range second.ToolEvents {
		if ev.SessionID != uuid {
			t.Fatalf("incremental event session id = %q, want %q (duplicate-session bug)", ev.SessionID, uuid)
		}
		if ev.ProjectRoot != "/home/me/proj" {
			t.Fatalf("incremental event project root = %q, want /home/me/proj", ev.ProjectRoot)
		}
		if ev.Model != "openai-codex/gpt-5.5" {
			t.Fatalf("incremental event model = %q, want openai-codex/gpt-5.5", ev.Model)
		}
	}
	for _, tok := range second.TokenEvents {
		if tok.SessionID != uuid {
			t.Fatalf("incremental token session id = %q, want %q", tok.SessionID, uuid)
		}
		if tok.ProjectRoot != "/home/me/proj" {
			t.Fatalf("incremental token project root = %q, want /home/me/proj", tok.ProjectRoot)
		}
	}
}
