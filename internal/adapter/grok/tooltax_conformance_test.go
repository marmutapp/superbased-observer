...
// re-implementation.
//
// The classifier is mapToolName (records.go), a normalized-key switch (lower-cased, `_`/`-` stripped).
func classifyForTooltax(native string) string {
	return mapToolName(native)
}

// tooltaxTools lists the tool ids this adapter's classifier feeds.
var tooltaxTools = []string{models.ToolGrok}
...
//
// The contract on the VALUE is enforced, not just its presence: it must
// start with the literal prefix "deliberate: " or "dead: ", followed by a
// non-empty explanation — see validUnclassifiedReason. An entry frozen with
// an empty (or otherwise unlabeled) reason is rejected, so the map can never
// silently regress to "acknowledged, no reason given".
var unclassifiedDomain = map[string]string{}

// validUnclassifiedReason enforces unclassifiedDomain's value contract: the
// reason must declare which of the two acknowledged shapes it is —
// "deliberate: <why>" or "dead: <why>" — with a non-empty <why>. A blank
// reason, or one missing both prefixes, is rejected.
func validUnclassifiedReason(reason string) bool {
	for _, prefix := range []string{"deliberate: ", "dead: "} {
		if strings.HasPrefix(reason, prefix) && strings.TrimSpace(strings.TrimPrefix(reason, prefix)) != "" {
			return true
		}
	}
	return false
...
func TestClassifierDomainIsSubsetOfTooltax(t *testing.T) {
	...
	for _, native := range domain {
		...
		if got == "" || got == models.ActionUnknown {
			reason, frozen := unclassifiedDomain[native]
			if !frozen {
			...
			} else if !validUnclassifiedReason(reason) {
				t.Errorf("unclassifiedDomain[%q] = %q does not start with %q or %q "+
					"followed by a non-empty explanation — an acknowledged unclassified "+
					"case must say which of the two it is, and why", native, reason,
					"deliberate: ", "dead: ")
			}
			continue
		}
