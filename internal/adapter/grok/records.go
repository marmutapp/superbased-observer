...
type acpUpdate struct {
	...
	Content       acpContentField `json:"content"`
...
// acpContent is one content block of an ACP update's `content` field. The
// field is polymorphic across variants (live-verified 2026-07-09):
//
//   - message chunks (user_message_chunk / agent_message_chunk /
//     agent_thought_chunk) carry a SINGLE text object:
//     {"type":"text","text":"…"}
//   - a tool_call_update carries an ARRAY of output blocks, each one of:
//     {"type":"content","content":{"type":"text","text":"…"}}   (tool output)
//     {"type":"diff","path":"…","oldText":"…","newText":"…"}     (write/edit diff)
//
// A single struct decodes every block: the nested Content pointer resolves
// the {"type":"content",…} wrapper, and the diff fields capture the
// write/edit shape. textValue walks the wrapper to the leaf text.
...
// textValue extracts the human-readable text of a single block, resolving
// the {"type":"content","content":{…}} wrapper one level down. A diff block
// (no text) yields "".
func (c *acpContent) textValue() string {
	if c == nil {
		return ""
	}
	...
	if c.Content != nil {
		return c.Content.textValue()
	}
	return ""
}

// acpContentField is the polymorphic `content` field of an ACP update. It
// decodes BOTH a single content object (message chunks) AND an array of
// content blocks (tool_call_update output) into the same []acpContent
// slice, so neither shape is rejected — before this seam an array-shaped
// content aborted the whole line's unmarshal ("cannot unmarshal array into
// Go struct field acpUpdate.params.update.content"), silently dropping the
// terminal tool status + output (live bug, Windows bundle line 65,
// 2026-07-09).
type acpContentField []acpContent

// UnmarshalJSON accepts a single object, an array of objects, a bare
// string, or null. Anything else returns an error so a genuinely novel
// shape still surfaces as a malformed-line warning rather than being
// silently dropped.
func (f *acpContentField) UnmarshalJSON(data []byte) error {
	...
	case '{':
		...
		*f = acpContentField{one}
	case '"':
		var s string
		if err := json.Unmarshal(trimmed, &s); err != nil {
			return err
		}
		*f = acpContentField{{Type: "text", Text: s}}
	default:
		return fmt.Errorf("grok: unexpected content shape starting with %q", string(trimmed[0]))
	}
	return nil
}

// firstText returns the text of the first block that carries any, resolving
// nested wrappers. Message chunks carry a single block, so this yields
// their text.
func (f acpContentField) firstText() string {
	for i := range f {
		if t := f[i].textValue(); t != "" {
			return t
		}
	}
	return ""
...
// joinedText concatenates the text of every block (a tool_call_update's
// output can span multiple blocks). Diff blocks (no text) are skipped.
func (f acpContentField) joinedText() string {
	var parts []string
	for i := range f {
		if t := strings.TrimSpace(f[i].textValue()); t != "" {
...
func mapToolName(name string) string {
	...
	case "editfile", "edit", "replace", "applypatch", "patch", "strreplace",
		"searchreplace", "multiedit", "deletefile", "removefile":
		// searchreplace = grok's live edit tool (title="search_replace",
		// rawInput {file_path, old_string, new_string}); before it was
		// added this was the session's lone `unknown` action (live capture
		// 2026-07-09). File deletion follows the copilot precedent
		// (deletefile → edit_file, a file mutation); grok's own delete
		// happens via run_terminal_command (`rm`/`Remove-Item`), so these
		// tool names are defensive — no distinct grok delete tool observed.
