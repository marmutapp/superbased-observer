...
// and its `-wal` / `-shm` sidecars are scanned by a `messages.id`
// watermark, so the persisted cursor is a row id rather than a byte
// offset.
func (a *Adapter) CursorSemanticsFor(path string) adapter.FileCursorSemantics {
	...
	return adapter.FileCursorSemantics{
		...
		Detail: "goose sessions.db is scanned by a messages.id watermark; the cursor is not a byte offset",
	}
}
