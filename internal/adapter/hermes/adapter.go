...
//
// Hook + watcher running in tandem MAY produce two rows per turn
// (one from each path) sharing SessionID = sessions.id but with
// distinct SourceFile values: hook rows carry "hermes:hook" while
// watcher rows carry the absolute path to state.db. The (SourceFile,
// SourceEventID) UNIQUE index makes this idempotent — the watcher's
// re-scan never inserts duplicates against itself.
//
...
func (a *Adapter) ParseSessionFile(ctx context.Context, path string, fromOffset int64) (adapter.ParseResult, error) {
	...
	toolEvents, tokenEvents, warnings := buildEvents(sessions, messages, dbPath, a.scrubber)
