...
// re-implementation.
//
// The classifier is normalizeToolName (normalize.go), an exact-name switch behind `mcp_`/`browser_` prefix rules (prefix rules are not a finite domain and are outside this pin).
...
var tooltaxTools = []string{models.ToolHermes}
