...
// re-implementation.
//
// The classifier is mapToolName (parser.go), a normalized-key switch.
...
var tooltaxTools = []string{models.ToolGeminiCLI}
...
var unclassifiedDomain = map[string]string{
	"updatetopic": "deliberate: sets a running conversation-topic label " +
		"(live args: title, summary, strategic_intent) — a narrative, " +
		"not a structured todo-list/plan with discrete items and " +
		"per-item status like ActionTodoUpdate's contract requires " +
		"(codex update_plan, TodoWrite, etc.); no dedicated type exists " +
		"for topic narration.",
}
