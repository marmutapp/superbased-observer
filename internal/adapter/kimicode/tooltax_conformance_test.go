...
// re-implementation.
//
// The classifier is mapKimiTool (records.go).
func classifyForTooltax(native string) string {
	return mapKimiTool(native)
}

// tooltaxTools lists the tool ids this adapter's classifier feeds.
var tooltaxTools = []string{models.ToolKimiCode}
...
func TestClassifierDomainIsSubsetOfTooltax(t *testing.T) {
	domain, err := conformast.SwitchCaseStrings(".", "mapKimiTool")
	if err != nil {
