...
// re-implementation.
//
// The classifier is mapTool (adapter.go), a `switch strings.ToLower(strings.TrimSpace(name))`.
func classifyForTooltax(native string) string {
	return func() string { a, _ := mapTool(native, nil); return a }()
}

// tooltaxTools lists the tool ids this adapter's classifier feeds.
var tooltaxTools = []string{models.ToolDevin}
