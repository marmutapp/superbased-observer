...
// re-implementation.
//
// The classifier is mapToolName (events.go), the bare-name half of classifyToolName.
...
var tooltaxTools = []string{models.ToolCopilotCLI}
...
var unclassifiedDomain = map[string]string{
	// DELIBERATE: `report_intent` is Copilot CLI narrating what it is about
	// to do — a harness affordance, not agent work on a target. tooltax
	// mirrors it as an explicit ActionUnknown row.
	"report_intent": "deliberate: harness narration, not a targeted action",
}
