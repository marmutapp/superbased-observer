...
func TestParseSessionFile_FixtureCorpus(t *testing.T) {
	...
	// TokenEvents: 1 session-level aggregate + 3 per-message (one
	// per assistant message with a metrics block). Phase 0 reality-
	// check upgrade.
	if got, want := len(res.TokenEvents), 4; got != want {
		t.Errorf("len(TokenEvents) = %d; want %d (1 session + 3 per-message)", got, want)
	}

	// Spot-check: the session-level token row carries the
	// metadata.usage.totalCost.
	var foundSessionToken bool
	for _, te := range res.TokenEvents {
		if te.SourceEventID == "tk:"+fixtureSessionID+":session_usage" {
			foundSessionToken = true
			if te.InputTokens != 286579 {
				t.Errorf("session-level InputTokens = %d; want 286579", te.InputTokens)
			}
			if te.EstimatedCostUSD < 0.02 || te.EstimatedCostUSD > 0.03 {
				t.Errorf("session-level cost = %v; want ~0.020247", te.EstimatedCostUSD)
			}
		}
	}
	if !foundSessionToken {
		t.Error("session-level token row not found")
	}
