...
//     Tier 2 approximate; superseded by per-message metrics rows
//     downstream via the store's MAX-upgrade on token conflict
//
...
// metadata_json.usage. Tier 2 approximate. Per-message metrics rows
// downstream will share the SessionID but distinct
// SourceEventIDs ("m<id>:metrics"), so both coexist in token_usage
