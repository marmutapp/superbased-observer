...
func TestFindProjectDB_MatchesProject(t *testing.T) {
	...
	// Build the project root and stored file_path with the platform's
	// native separator: probeProjectMatch's LIKE prefix uses
	// filepath.Separator, so a POSIX-literal projRoot would never match
	// on Windows (the prefix becomes "...myproject\" but the stored path
	// uses "/"). Native separators keep the prefix-match meaningful on
	// every OS.
	projRoot := filepath.Join(string(filepath.Separator), "home", "user", "myproject")
	_, err = d.Exec(
		`INSERT INTO nodes (kind, name, file_path, start_line, end_line, language)
		 VALUES ('function', 'Main', ?, 1, 10, 'go')`,
		filepath.Join(projRoot, "main.go"),
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	if probeProjectMatch(dbPath, filepath.Join(string(filepath.Separator), "other", "project")) {
