...
func (s *Session) emitLease(kind LeaseEventKind, holder, reason string) {
	...
	ev := LeaseEvent{Kind: kind, Handle: s.handle, Holder: holder, Reason: reason, At: s.now()}
...
// Holder returns the lease holder identity ("local" or a device-session
// fingerprint) for the local-UI display + audit.
func (l *WriterLease) Holder() string { return l.holder }
...
// AcquireWriterRemote.
func (s *Session) acquireWriter(req termlease.Requester, holder string) (*WriterLease, error) {
	s.writeMu.Lock()
...
func (m *Manager) AcquireWriterLocal(handle string) (*WriterLease, error) {
	...
	return s.acquireWriter(termlease.RequesterLocal, "local")
}
...
func (m *Manager) AcquireWriterRemote(handle string, grant termlease.WriterGrant) (*WriterLease, error) {
	...
	return s.acquireWriter(termlease.RequesterRemote, grant.Holder())
}
...
// RevokeRemoteWriterByHolder revokes the REMOTE writer lease held by a specific
// device fingerprint (grant.Holder(), i.e. sha256(device-session)[:8]), if any,
// through the funnel — the single-device kill the dashboard drives for a device-
// session revoke. The owner-LOCAL loopback writer (holder "local") is never
// matched. Returns whether a lease was revoked.
func (m *Manager) RevokeRemoteWriterByHolder(fingerprint, reason string) bool {
	...
	for _, s := range all {
		s.writeMu.Lock()
		l := s.lease
		match := l != nil && l.kind == termlease.HolderRemote && l.holder == fingerprint
		s.writeMu.Unlock()
		if match {
			s.revokeLease(l, LeaseRevoked, reason)
			revoked = true
...
// WriterHolder returns the current writer-lease holder identity for a handle
// ("local" or a device-session fingerprint) and whether a writer is held. Used
// by the local UI to surface the controlling device.
...
func (m *Manager) Create(spec Spec) (string, error) {
	...
	default:
		...
		// for a handoff launch (a fresh launch has no source session — F1).
		if spec.BinPath == "" || spec.Subcommand == "" {
			return "", ErrInvalidSpec
		}
		if !spec.Fresh && spec.SessionID == "" {
			return "", ErrInvalidSpec
		}
	}
