...
func TestWalkStopsAtMaxDepth(t *testing.T) {
	// Build a payload nested far deeper than maxWalkDepth: each level is a
	// bytes field wrapping the level below. Without the depth cap this
	// recurses until the goroutine stack overflows (a fatal crash); with it,
	// Walk terminates and simply stops descending past the cap.
	const nesting = maxWalkDepth + 40
	buf := AppendVarintField(nil, 1, 7) // innermost leaf
	for i := 0; i < nesting; i++ {
		buf = AppendBytesField(nil, 1, buf)
	}

	maxDepthSeen := 0
	err := Walk(buf, func(f Field) error {
		if f.Depth > maxDepthSeen {
			maxDepthSeen = f.Depth
		}
		return nil
	})
	if err != nil {
	...
	// Depth is capped: the deepest field yielded is bounded by maxWalkDepth,
	// not by the (much larger) actual nesting.
	if maxDepthSeen >= nesting {
		t.Errorf("descent was not capped: max depth seen %d, nesting %d", maxDepthSeen, nesting)
	}
	if maxDepthSeen > maxWalkDepth {
		t.Errorf("max depth seen %d exceeds cap %d", maxDepthSeen, maxWalkDepth)
	}
}
