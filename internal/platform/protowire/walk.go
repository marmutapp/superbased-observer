...
// maxWalkDepth bounds submessage recursion in Walk. Real protobuf messages
// nest only a handful of levels deep; the cap is a DoS guard against a
// hostile self-nesting payload overflowing the goroutine stack (a fatal Go
// crash). Fields at the cap are still yielded; only the descent stops.
...
func walk(buf []byte, path []int, yield func(Field) error) error {
	...
	for pos < len(buf) {
		...
		// Recurse into submessage when the payload looks like nested wire
		// format — but stop at maxWalkDepth. Without the cap, a deeply-nested
		// (or adversarially self-nesting) payload synced into a watched store
		// would recurse until the goroutine stack overflows, which in Go is a
		// fatal, unrecoverable crash of the whole daemon. Beyond the cap the
		// field is still yielded as a leaf (its raw Bytes are available to the
		// caller); only the descent stops.
		if wt == WireBytes && len(field.Path) < maxWalkDepth && shouldRecurse(field.Bytes) {
