...
func TestNewSweeper_NilWhenDisabled(t *testing.T) {
	d := openDB(t)
	if s := NewSweeper(d, 0, 0, nil); s != nil {
		t.Errorf("NewSweeper(0,0) = non-nil, want nil (all retention off)")
	}
	if s := NewSweeper(d, 30, 0, nil); s == nil {
		t.Errorf("NewSweeper(30,0) = nil, want a sweeper (content horizon on)")
	}
	if s := NewSweeper(d, 0, 30, nil); s == nil {
		t.Errorf("NewSweeper(0,30) = nil, want a sweeper (data horizon on)")
	}
	if s := NewSweeper(d, 30, 90, nil); s == nil {
		t.Errorf("NewSweeper(30,90) = nil, want a sweeper (both on)")
	}
}
