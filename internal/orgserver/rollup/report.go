...
// WindowReportResult is the content-free spend rollup for an ARBITRARY
// [start, end) window, powering the scheduled digest (gap-register G13). It
// mirrors ReportResult's dimensions but takes explicit bounds (so a weekly
// digest works, not just a calendar month) and adds ByDeveloper — the
// email-attributed per-dev breakdown the monthly chargeback statement omits.
...
// WindowReport builds the spend rollup for [start, end), scoped to the caller.
// It reuses the same spend-CTE data assembly as the monthly Report (never the
// HTTP layer), so the digest and the on-demand statement agree. Content-free:
// project keys are hash-derived ids, developer keys are member emails.
func WindowReport(ctx context.Context, db *sql.DB, scope Scope, start, end, now time.Time) (WindowReportResult, error) {
	startRFC := start.UTC().Format(time.RFC3339)
	endRFC := end.UTC().Format(time.RFC3339)
	uScope, uArgs := userScopeSQL("user_id", scope)
	res := WindowReportResult{
		Start: start, End: end,
		ByModel: []KeyCost{}, ByTool: []KeyCost{}, ByProject: []KeyCost{}, ByDeveloper: []KeyCost{},
	}

	byModel, err := reportDim(ctx, db, "model", startRFC, endRFC, uScope, uArgs, false)
	if err != nil {
		return WindowReportResult{}, fmt.Errorf("rollup.WindowReport: by model: %w", err)
	}
	res.ByModel = byModel
	byTool, err := reportDim(ctx, db, "tool", startRFC, endRFC, uScope, uArgs, false)
	if err != nil {
		return WindowReportResult{}, fmt.Errorf("rollup.WindowReport: by tool: %w", err)
	}
	res.ByTool = byTool
	byProj, err := reportDim(ctx, db, "project_root_hash", startRFC, endRFC, uScope, uArgs, true)
	if err != nil {
		return WindowReportResult{}, fmt.Errorf("rollup.WindowReport: by project: %w", err)
	}
	res.ByProject = byProj
	byDev, err := reportDeveloperDim(ctx, db, startRFC, endRFC, scope)
	if err != nil {
		return WindowReportResult{}, fmt.Errorf("rollup.WindowReport: by developer: %w", err)
	}
	res.ByDeveloper = byDev
	for _, m := range byModel {
		res.TotalUSD += m.CostUSD
	}
	return res, nil
...
// reportDeveloperDim sums spend + tokens per developer (member email) within
// [start, end), joining spend rows to org_members. Rows with no member match
// fall back to the raw user_id so spend is never silently dropped.
func reportDeveloperDim(ctx context.Context, db *sql.DB, start, end string, scope Scope) ([]KeyCost, error) {
	uScope, uArgs := userScopeSQL("sp.user_id", scope)
	//nolint:gosec // G202: spendCTE is a code constant; uScope parameterized; values bind via ?.
	q := spendCTE + `
SELECT COALESCE(NULLIF(m.email,''), sp.user_id, '') AS dev,
       COALESCE(SUM(sp.cost),0), COALESCE(SUM(sp.tokens),0)
  FROM spend sp
  LEFT JOIN org_members m ON m.user_id = sp.user_id
 WHERE sp.ts < ? AND ` + uScope + `
 GROUP BY dev
 ORDER BY 2 DESC`
