...
const clientSecretEnv = "M365_COPILOT_CLIENT_SECRET" //nolint:gosec // G101: env-var NAME, not a credential
...
// secret in an error. When the secret is sourced from a file, it emits a WARN via
// logger if the file is group/world-readable (see warnSecretFilePerms); an
// env-sourced secret has no file to check. A nil logger falls back to
// slog.Default(). The warning never fails the read — the secret is still usable.
func ResolveClientSecret(secretFile string, logger *slog.Logger) (string, error) {
	...
	if secretFile != "" {
		...
		warnSecretFilePerms(secretFile, logger)
...
// warnSecretFilePerms emits a WARN when the client secret file is group- or
// world-accessible (mode & 0o077 != 0), naming the file and recommending
// chmod 600. It never fails and never logs the secret contents — only the path
// and the octal mode. A stat error is ignored (the read already succeeded; a
// missing-perms probe must not mask a usable secret).
func warnSecretFilePerms(path string, logger *slog.Logger) {
	if logger == nil {
		logger = slog.Default()
	}
	...
	if perm := info.Mode().Perm(); perm&0o077 != 0 {
		logger.Warn("m365-copilot client_secret_file is group/world-readable; restrict it to the server user",
			"file", path, "mode", fmt.Sprintf("%04o", perm), "recommended", "chmod 600")
	}
}
