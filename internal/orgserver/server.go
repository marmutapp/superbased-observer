...
import (
	...
	"github.com/marmutapp/superbased-observer/internal/orgserver/digest"
...
// AppVersion is the org-server build version stamped into outbound artifacts
// (currently the [email] alert footer, gap-register G9). cmd/observer-org sets
// it from its own version var at startup; it defaults to "dev".
var AppVersion = "dev"
...
type Server struct {
	...
	// m365Schedulers are the OPTIONAL Microsoft 365 Copilot pollers (native-console
	// instance #4 / World 1) — ONE per configured rail (graph | purview). Empty
	// unless [m365_copilot].enabled and an Entra client secret resolved; each runs
	// alongside the HTTP server. MICROSOFT's M365 Copilot, NOT GitHub Copilot.
	m365Schedulers []*m365copilotanalytics.Scheduler
	...
	// digestScheduler is the OPTIONAL scheduled report digest (gap-register
	// G13). Non-nil only when [digest].enabled AND the shared [email] notifier
	// is present; runs alongside the HTTP server like the evaluators.
...
func New(ctx context.Context, cfg config.Config, logger *slog.Logger) (*Server, error) {
	...
	// delivery channel for the alert evaluators. Built once when [email] is
	// enabled; a bad [email] block is WARN-only (Validate already ran, so this
	// is belt-and-suspenders) and leaves the notifier nil (email simply off).
	// Each evaluator opts in independently via its [dashboard] toggle.
	var emailNotifier *email.Notifier
	if cfg.Email.Enabled {
		if n, err := email.NewNotifier(cfg.Email, logger); err != nil {
			logger.Warn("email notifier disabled: invalid [email] config", "err", err)
		} else {
			emailNotifier = n
		}
	}
	...
	if cfg.Dashboard.BudgetAlertEmail {
		evaluator.Email = emailNotifier
		evaluator.Version = AppVersion
	}
	...
	obsAlertEval := obsalert.NewEvaluator(db, org,
		time.Duration(cfg.Dashboard.BudgetPollSeconds)*time.Second, logger)
	if cfg.Dashboard.ObsAlertEmail {
		obsAlertEval.Email = emailNotifier
		obsAlertEval.Version = AppVersion
	}
	...
	ccScheduler := buildCCScheduler(cfg, db, org.OrgID, logger)
	...
	codexScheduler := buildCodexScheduler(cfg, db, org.OrgID, logger)
	...
	copilotSchedulers := buildCopilotSchedulers(cfg, db, org.OrgID, logger)

	// M365 Copilot pollers (native-console instance #4 / World 1) — same posture
	// as CC/Codex/Copilot, ONE scheduler per configured rail (graph | purview). A
	// missing client secret or a bad rail is WARN-only and skips that rail; the
	// others still run, and nothing blocks startup. MICROSOFT's M365 Copilot.
	m365Schedulers := buildM365Schedulers(cfg, db, org.OrgID, logger)

	// Retention sweeper — nil unless a positive horizon is set on EITHER knob.
	// [dashboard.content_retention].otel_content_days (default 0 = keep forever)
	// NULLs stored message bodies; server.data_retention_days (default 730 —
	// enforced since the gap-audit #12 fix, previously validated-only) DELETES
	// aged rows across the pushed-data tables. NewSweeper returns nil only when
	// both are ≤0.
	retentionSweeper := retention.NewSweeper(db, cfg.Dashboard.ContentRetention.OTelContentDays, cfg.Server.DataRetentionDays, logger)

	// Digest scheduler (gap-register G13) — built only when [digest].enabled AND
	// the shared [email] notifier is present (a digest is delivered by email, so
	// with no mail channel there is nothing to schedule). Default OFF.
	var digestScheduler *digest.Scheduler
	if cfg.Digest.Enabled && emailNotifier != nil {
		digestScheduler = digest.NewScheduler(db, org, cfg.Digest, emailNotifier, AppVersion, cfg.Email.To, logger)
	} else if cfg.Digest.Enabled {
		logger.Warn("digest disabled: [digest].enabled set but [email] is not enabled")
	}

	return &Server{cfg: cfg, db: db, org: org, logger: logger, handler: handler, evaluator: evaluator, obsAlertEval: obsAlertEval, ccScheduler: ccScheduler, codexScheduler: codexScheduler, copilotSchedulers: copilotSchedulers, m365Schedulers: m365Schedulers, retentionSweeper: retentionSweeper, digestScheduler: digestScheduler}, nil
}

// buildCCScheduler constructs the CC Analytics poller/scheduler (native-console
// Workstream C) — nil unless enabled AND a key resolves AND the api_kind
...
func buildCCScheduler(cfg config.Config, db *sql.DB, orgID string, logger *slog.Logger) *ccanalytics.Scheduler {
	...
	if _, verr := ccanalytics.ValidateKind(cfg.CCAnalytics.ApiKind); verr != nil {
		logger.Warn("cc-analytics disabled: bad api_kind", "err", verr)
		return nil
	}
	poller := &ccanalytics.Poller{
		DB: db, BaseURL: cfg.CCAnalytics.BaseURL, APIKey: key, OrgID: orgID,
	}
	return ccanalytics.NewScheduler(poller,
		cfg.CCAnalytics.PollIntervalHours, cfg.CCAnalytics.LagToleranceHours, logger)
}

// buildCodexScheduler constructs the Codex Analytics poller/scheduler
// (native-console instance #2, Rail C) — same WARN-only posture as CC. Extracted
// from New to keep its cyclomatic complexity in check.
func buildCodexScheduler(cfg config.Config, db *sql.DB, orgID string, logger *slog.Logger) *codexanalytics.Scheduler {
	...
	poller, perr := codexanalytics.NewPoller(db, cfg.CodexAnalytics.Surface,
		cfg.CodexAnalytics.BaseURL, key, cfg.CodexAnalytics.WorkspaceID, orgID)
	if perr != nil {
		logger.Warn("codex-analytics disabled: bad config", "err", perr)
		return nil
	}
	return codexanalytics.NewScheduler(poller,
		cfg.CodexAnalytics.PollIntervalHours, cfg.CodexAnalytics.LagToleranceHours, logger)
}

// buildCopilotSchedulers constructs one Copilot Analytics scheduler per
// configured surface (native-console instance #3, Rail C) — WARN-only per
// surface; a bad surface skips only itself. Extracted from New to keep its
// cyclomatic complexity in check.
func buildCopilotSchedulers(cfg config.Config, db *sql.DB, orgID string, logger *slog.Logger) []*copilotanalytics.Scheduler {
	...
	var schedulers []*copilotanalytics.Scheduler
	for _, surface := range cfg.CopilotAnalytics.Surfaces {
		poller, perr := copilotanalytics.NewPoller(db, surface, cfg.CopilotAnalytics.BaseURL,
			...
			cfg.CopilotAnalytics.Report, orgID)
		if perr != nil {
			logger.Warn("copilot-analytics surface skipped", "surface", surface, "err", perr)
			continue
		}
		schedulers = append(schedulers, copilotanalytics.NewScheduler(poller,
			cfg.CopilotAnalytics.PollIntervalHours, cfg.CopilotAnalytics.LagToleranceHours, logger))
	}
	return schedulers
}

// buildM365Schedulers constructs one M365 Copilot scheduler per configured
// surface (native-console instance #4 / World 1) — WARN-only per surface.
// Extracted from New to keep its cyclomatic complexity in check.
func buildM365Schedulers(cfg config.Config, db *sql.DB, orgID string, logger *slog.Logger) []*m365copilotanalytics.Scheduler {
	if !cfg.M365Copilot.Enabled {
		return nil
	}
	secret, serr := m365copilotanalytics.ResolveClientSecret(cfg.M365Copilot.ClientSecretFile, logger)
	if serr != nil {
		logger.Warn("m365-copilot disabled: no Entra client secret", "err", serr)
		return nil
	}
	tokenSrc := m365copilotanalytics.NewTokenSource(
		cfg.M365Copilot.TenantID, cfg.M365Copilot.ClientID, secret, cfg.M365Copilot.LoginBaseURL, nil,
	)
	var schedulers []*m365copilotanalytics.Scheduler
	for _, surface := range cfg.M365Copilot.Surfaces {
		poller, perr := m365copilotanalytics.NewPoller(db, surface,
			cfg.M365Copilot.GraphBaseURL, tokenSrc, cfg.M365Copilot.TenantID, orgID)
		if perr != nil {
			logger.Warn("m365-copilot surface skipped", "surface", surface, "err", perr)
			continue
		}
		poller.StoreContent = cfg.M365Copilot.StoreContent
		schedulers = append(schedulers, m365copilotanalytics.NewScheduler(poller,
			cfg.M365Copilot.PollIntervalHours, cfg.M365Copilot.LagToleranceHours, logger))
	}
	return schedulers
}
...
func (s *Server) Run(ctx context.Context) error {
	...
	// M365 Copilot pollers (optional; one per rail) run alongside; each stops on
	// ctx cancel.
	for _, sched := range s.m365Schedulers {
		go sched.Run(ctx)
	}
	...
	// Digest scheduler (optional) runs alongside; stops on ctx cancel.
	if s.digestScheduler != nil {
		go s.digestScheduler.Run(ctx)
	}
...
func buildMux(d buildDeps) http.Handler {
	...
	// T6 input-admission monitoring (Plane-A admission org tier, gap-audit #1b)
	// — admin surface, READ-ONLY, JSON only (manual routes, mirroring
	// enduser-spend). /reasons is a DEEPER, server-audited disclosure.
	mux.Handle("GET /api/org/obs/admission", requireAdminSAML(obsAdmissionHandler(d.db)))
	mux.Handle("GET /api/org/obs/admission/reasons", requireAdminSAML(obsAdmissionReasonsHandler(d.db)))
	// T7 per-item eval detail (Plane-A eval-run detail org tier, gap-audit §1 /
	// §2.2 / §6) — admin surface, READ-ONLY, JSON only (manual routes, mirroring
	// admission). /run/content is a DEEPER, server-audited disclosure. NOTE the
	// route order: the longer static ".../run/content" is registered BEFORE
	// ".../run" so net/http's ServeMux longest-pattern-wins rule routes the
	// content read to its own audited handler.
	mux.Handle("GET /api/org/obs/eval/runs", requireAdminSAML(obsEvalRunsHandler(d.db)))
	mux.Handle("GET /api/org/obs/eval/run/content", requireAdminSAML(obsEvalRunContentHandler(d.db)))
	mux.Handle("GET /api/org/obs/eval/run", requireAdminSAML(obsEvalRunHandler(d.db)))
	mux.Handle("GET /api/org/obs/eval/compare", requireAdminSAML(obsEvalCompareHandler(d.db)))
