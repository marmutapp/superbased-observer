...
func New(ctx context.Context, cfg config.Config, logger *slog.Logger) (*Server, error) {
	...
	var m365Schedulers []*m365copilotanalytics.Scheduler
	if cfg.M365Copilot.Enabled {
		secret, serr := m365copilotanalytics.ResolveClientSecret(cfg.M365Copilot.ClientSecretFile)
		if serr != nil {
			...
			for _, surface := range cfg.M365Copilot.Surfaces {
				poller, perr := m365copilotanalytics.NewPoller(db, surface,
					cfg.M365Copilot.GraphBaseURL, tokenSrc, cfg.M365Copilot.TenantID, org.OrgID)
				if perr != nil {
					logger.Warn("m365-copilot surface skipped", "surface", surface, "err", perr)
					continue
				}
				poller.StoreContent = cfg.M365Copilot.StoreContent
				m365Schedulers = append(m365Schedulers, m365copilotanalytics.NewScheduler(poller,
