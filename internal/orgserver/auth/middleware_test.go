...
func TestRequireAdminSAML(t *testing.T) {
	sessions := newTestSessions(t)
	admins := map[string]bool{"admin-1": true}
	isAdmin := func(_ context.Context, userID string) (bool, error) {
		return admins[userID], nil
	}
	guarded := RequireAdminSAML(sessions, isAdmin, JSONUnauthorized(), JSONForbidden())(okHandler())

	// Admin session → through.
	rec := httptest.NewRecorder()
	guarded.ServeHTTP(rec, roundTrip(t, sessions, "admin-1"))
	if rec.Code != http.StatusOK {
		t.Errorf("admin session: code=%d, want 200", rec.Code)
	}

	// Valid session but non-admin → 403, not 200.
	rec = httptest.NewRecorder()
	guarded.ServeHTTP(rec, roundTrip(t, sessions, "member-9"))
	if rec.Code != http.StatusForbidden {
		t.Errorf("non-admin session: code=%d, want 403", rec.Code)
	}
	...
	// Checker error → treated as non-admin (fail-closed), 403.
	failChecker := func(_ context.Context, _ string) (bool, error) {
		return false, errors.New("db down")
	}
	rec = httptest.NewRecorder()
	RequireAdminSAML(sessions, failChecker, JSONUnauthorized(), JSONForbidden())(okHandler()).
		ServeHTTP(rec, roundTrip(t, sessions, "admin-1"))
	if rec.Code != http.StatusForbidden {
		t.Errorf("checker error: code=%d, want 403 (fail-closed)", rec.Code)
	}
}
