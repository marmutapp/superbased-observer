...
// TestEvaluator_EmailFailureIsFailSoft attaches a real email notifier whose
// SMTP send always fails (refused loopback port) and asserts the obs-alert
// evaluation still fires the webhook, logs the event, and returns nil — email
// is an additional, fail-soft channel that never blocks evaluation.
func TestEvaluator_EmailFailureIsFailSoft(t *testing.T) {
	ctx := context.Background()
	db, err := orgdb.Open(ctx, orgdb.Options{Path: filepath.Join(t.TempDir(), "server.db")})
	if err != nil {
		t.Fatalf("orgdb.Open: %v", err)
	...
	e.Version = "v-test"
	fired := 0
	e.Deliver = func(_ context.Context, _ string, _ Alert) error { fired++; return nil }

	if err := e.EvaluateOnce(ctx); err != nil {
		t.Fatalf("EvaluateOnce must not fail when email fails: %v", err)
	}
	if fired != 1 {
		t.Errorf("webhook fired %d times, want 1", fired)
	}
	res, err := LoadAlertRules(ctx, db, org.OrgID)
	if err != nil {
		t.Fatalf("LoadAlertRules: %v", err)
	}
	if len(res.Events) != 1 || !res.Events[0].Delivered {
		t.Errorf("events = %+v, want 1 delivered fire despite email failure", res.Events)
	}
}
