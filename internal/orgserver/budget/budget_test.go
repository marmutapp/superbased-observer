...
// TestEvaluateOnce_EmailFailureIsFailSoft wires a real email notifier whose
// SMTP send always fails (a refused loopback port) and asserts that the alert
// evaluation still succeeds: the webhook fires, the high-water mark persists,
// and EvaluateOnce returns nil. Email is an additional, fail-soft channel.
func TestEvaluateOnce_EmailFailureIsFailSoft(t *testing.T) {
	d := newEvalDB(t)
	ctx := context.Background()
	if _, err := d.ExecContext(ctx,
		`INSERT INTO budgets (id, scope, scope_id, monthly_usd_cap, alert_webhook_url, alert_thresholds, last_fired_threshold, created_at, updated_at)
	...
	// A notifier pointed at a refused loopback port: Send fails fast, fail-soft.
	notifier, err := email.NewNotifier(email.Config{
		Enabled: true, Host: "127.0.0.1", Port: 1, From: "obs@x",
		To: []string{"admin@x"}, TLSMode: email.TLSNone, TimeoutSeconds: 2,
	}, nil)
	if err != nil {
	...
	e.Version = "v-test"
	fired := 0
	e.Deliver = func(_ context.Context, _ string, _ gen.BudgetAlert) { fired++ }

	if err := e.EvaluateOnce(ctx); err != nil {
		t.Fatalf("EvaluateOnce must not fail when email delivery fails: %v", err)
	}
	if fired != 1 {
		t.Errorf("webhook fired %d times, want 1 (email failure must not suppress it)", fired)
	}
	var last float64
	_ = d.QueryRow(`SELECT last_fired_threshold FROM budgets WHERE id='b1'`).Scan(&last)
	if last != 0.75 {
		t.Errorf("last_fired_threshold = %v, want 0.75 (persisted despite email failure)", last)
	}
}
