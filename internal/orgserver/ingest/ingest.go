...
func ingestObsTiers(ctx context.Context, tx *sql.Tx, env orgcontract.PushEnvelope, userID, pushedAt string) error {
	...
	// T6 — input-admission verdict events (Plane-A admission org tier). Verdicts
	// are immutable and node-hash-chained, so they INSERT OR IGNORE on the
	// (org_id, user_email, row_hash) dedup key — a re-pushed window is a no-op.
	// user_email is re-pinned to the pusher (via forcePusherEmail); end_user is
	// app-shared and NOT re-pinned. The content-bearing columns (tenant,
	// end_user, reason_excerpt) are stored NULL when empty so the server cannot
	// tell "stripped" from "never had one" (no posture leak), exactly like
	// obs_content; the content-free chain/soft-join columns store their empty
	// string verbatim.
	for _, r := range env.ObsAdmissionEvents {
		if r.RowHash == "" {
			continue // nothing to anchor the dedup key on
		}
		if _, err := tx.ExecContext(ctx,
			`INSERT OR IGNORE INTO obs_admission_events
			   (org_id, user_email, row_hash, prev_hash, ts, mode, decision, severity,
			    ...
			    trace_id, session_id, request_id, message_hash,
			    tenant, end_user, reason_excerpt, pushed_at, pushed_by_user_id)
			 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
			r.OrgID, r.UserEmail, r.RowHash, r.PrevHash, r.TS, r.Mode, r.Decision, r.Severity,
			r.CriterionID, r.PolicyHash, boolToInt(r.JudgeUsed), r.JudgeHosting, r.Degraded, r.LatencyMS,
			r.TraceID, r.SessionID, r.RequestID, r.MessageHash,
			nullIfEmpty(r.Tenant), nullIfEmpty(r.EndUser), nullIfEmpty(r.ReasonExcerpt),
			pushedAt, userID); err != nil {
			return fmt.Errorf("ingest.Push: obs_admission_event %s: %w", r.RowHash, err)
		}
	}
	// T6 — admission policy snapshots. Content-addressed by policy_hash, so a
	// re-push UPSERTs (idempotent). body is the admin's authored policy (config)
	// and always ships; user requests never land here.
	for _, r := range env.ObsAdmissionPolicies {
		if r.PolicyHash == "" {
			continue
		}
		if _, err := tx.ExecContext(ctx,
			`INSERT INTO obs_admission_policy_versions
			   (org_id, user_email, policy_hash, created_at, mode, scope, criteria_count, body,
			    pushed_at, pushed_by_user_id)
			 VALUES (?,?,?,?,?,?,?,?,?,?)
			 ON CONFLICT(org_id, user_email, policy_hash) DO UPDATE SET
			   created_at=excluded.created_at, mode=excluded.mode, scope=excluded.scope,
			   criteria_count=excluded.criteria_count, body=excluded.body,
			   pushed_at=excluded.pushed_at`,
			r.OrgID, r.UserEmail, r.PolicyHash, r.CreatedAt, r.Mode, r.Scope, r.CriteriaCount, r.Body,
			pushedAt, userID); err != nil {
			return fmt.Errorf("ingest.Push: obs_admission_policy %s: %w", r.PolicyHash, err)
		}
	}
	// T7 — per-item eval scores (Plane-A eval-run detail org tier). UPSERT by the
	// (org_id, user_email, run_id, item_id, scorer) natural key so a re-pushed
	// window is idempotent AND a later full-content window can backfill the
	// excerpts onto a row that first arrived metadata-only. user_email is
	// re-pinned to the pusher (via forcePusherEmail). The content-bearing columns
	// (input/expected/output excerpts, rationale) are stored NULL when empty so
	// the server cannot tell "stripped" from "never had one" (no posture leak),
	// exactly like obs_content / obs_admission_events; the content-free score
	// metadata + content_hash store verbatim.
	for _, r := range env.ObsEvalItems {
		if _, err := tx.ExecContext(ctx,
			`INSERT INTO obs_eval_items
			   (org_id, user_email, run_id, run_name, dataset_id, dataset_name, item_id,
			    ...
			    pushed_at, pushed_by_user_id)
			 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
			 ON CONFLICT(org_id, user_email, run_id, item_id, scorer) DO UPDATE SET
			   run_name=excluded.run_name, dataset_id=excluded.dataset_id,
			   dataset_name=excluded.dataset_name, span_id=excluded.span_id,
			   trace_id=excluded.trace_id, score=excluded.score, passed=excluded.passed,
			   source=excluded.source, duration_ms=excluded.duration_ms, ts=excluded.ts,
			   content_hash=excluded.content_hash,
			   input_excerpt=excluded.input_excerpt, expected_excerpt=excluded.expected_excerpt,
			   output_excerpt=excluded.output_excerpt, rationale=excluded.rationale,
			   pushed_at=excluded.pushed_at`,
			r.OrgID, r.UserEmail, r.RunID, r.RunName, r.DatasetID, r.DatasetName, r.ItemID,
			r.SpanID, r.TraceID, r.Scorer, r.Score, boolToInt(r.Passed), r.Source, r.DurationMs, r.TS, r.ContentHash,
			nullIfEmpty(r.InputExcerpt), nullIfEmpty(r.ExpectedExcerpt), nullIfEmpty(r.OutputExcerpt), nullIfEmpty(r.Rationale),
			pushedAt, userID); err != nil {
			return fmt.Errorf("ingest.Push: obs_eval_item run=%d item=%d/%s: %w", r.RunID, r.ItemID, r.Scorer, err)
		}
	}
	return nil
}
...
func forcePusherEmail(env *orgcontract.PushEnvelope, email string) {
	...
	// T6 admission — re-pin the pusher on both slices. The verdict's EndUser is
	// app-shared (like ObsEndUserSpend.EndUser) and is NOT re-pinned; only
	// user_email is forced to the pusher so it cannot be forged.
	for i := range env.ObsAdmissionEvents {
		env.ObsAdmissionEvents[i].UserEmail = email
	}
	for i := range env.ObsAdmissionPolicies {
		env.ObsAdmissionPolicies[i].UserEmail = email
	}
	// T7 per-item eval — re-pin the pusher so user_email cannot be forged (it is
	// half the natural key that keeps each node's runs distinct).
	for i := range env.ObsEvalItems {
		env.ObsEvalItems[i].UserEmail = email
	}
}
