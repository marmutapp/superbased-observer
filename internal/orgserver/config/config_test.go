...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{"bad email tls mode", func(c *Config) {
...
// TestDumpRedactsEmailPassword pins the promise that Dump never emits a direct
// [email] password (only password_env / password_file references survive).
func TestDumpRedactsEmailPassword(t *testing.T) {
	cfg := Default()
	cfg.Server.ExternalURL = "https://org.example"
	cfg.Email.Enabled = true
	cfg.Email.Host = "smtp.example.com"
	cfg.Email.From = "obs@example.com"
	cfg.Email.Cred = "sup3rSecretMail"
	cfg.Email.CredEnv = "SMTP_PW"
	out, err := Dump(cfg)
	if err != nil {
		t.Fatalf("Dump: %v", err)
	}
	if strings.Contains(out, "sup3rSecretMail") {
		t.Errorf("dump leaked the email password:\n%s", out)
	}
	if !strings.Contains(out, "SMTP_PW") {
		t.Errorf("dump dropped the password_env reference:\n%s", out)
	}
}
