...
func checkOrgEnrolment(ctx context.Context, database *sql.DB, cfg config.Config) Check {
	...
	if err := database.QueryRowContext(ctx, `SELECT COUNT(*) FROM org_enrolment`).Scan(&enrolled); err != nil {
		return Check{Name: "org enrolment", Status: StatusFail, Message: "could not read org_enrolment table",
			Details: []string{err.Error()}}
	}
	if enrolled == 0 {
		return Check{Name: "org enrolment", Status: StatusWarn,
			Message: "[org_client] enabled but not enrolled — run `observer enroll --link <url>` to enrol",
		}
	}
	var (
		orgID, orgName, orgURL, userEmail string
	)
	if err := database.QueryRowContext(ctx,
		...
		return Check{Name: "org enrolment", Status: StatusFail, Message: "could not read enrolment row",
			Details: []string{err.Error()}}
	}
