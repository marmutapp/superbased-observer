...
// dropLocked removes an EXPIRED/superseded session: memory-first + best-effort
// durable delete. This is SAFE for the invariant because load-time expiry
// re-filters any TTL/idle-expired row that a failed delete leaves behind.
func (s *SessionStore) dropLocked(ds *deviceSession) {
	...
	if s.persister != nil {
		ctx, cancel := s.pctx()
		if err := s.persister.Delete(ctx, ds.id); err != nil {
