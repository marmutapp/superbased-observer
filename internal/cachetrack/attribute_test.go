...
// TestAttribute_HandoffRehydration proves the session-handoff row:
// the target session's first turn (Prior == nil) carrying the
// handoff marker attributes as cause=handoff_rehydration but keeps
// KindReanchor, while (a) a first turn WITHOUT the marker stays
// plain reanchor and (b) the marker on a LATER turn (Prior != nil,
// as it rides the conversation history) changes nothing.
func TestAttribute_HandoffRehydration(t *testing.T) {
	t.Parallel()

	// (a) first turn + marker → handoff_rehydration, same KindReanchor.
	firstWithMarker := Attribute(AttributeInput{
		Caps:                CapabilitiesFor(TierProxy),
		Model:               "claude-sonnet-4-6",
		Now:                 t0,
		CacheCreationTokens: 120000, // large cold rehydration write
		DivergedSeq:         -1,
		Prior:               nil,
		HandoffMarker:       true,
	})
	if firstWithMarker.Kind != KindReanchor || firstWithMarker.Cause != CauseHandoffRehydration {
		t.Errorf("first+marker: got (kind=%q, cause=%q), want (reanchor, handoff_rehydration)",
			firstWithMarker.Kind, firstWithMarker.Cause)
	}

	// (b) first turn WITHOUT marker → plain reanchor (unchanged).
	firstNoMarker := Attribute(AttributeInput{
		Caps:                CapabilitiesFor(TierProxy),
		Model:               "claude-sonnet-4-6",
		Now:                 t0,
	...
	if firstNoMarker.Kind != KindReanchor || firstNoMarker.Cause != CauseReanchor {
		t.Errorf("first+no-marker: got (kind=%q, cause=%q), want (reanchor, reanchor)",
			firstNoMarker.Kind, firstNoMarker.Cause)
	}

	// (c) marker on a LATER turn (Prior != nil) is inert — the rule
	// self-gates on Prior == nil, so a plain suffix-growth write is
	// attributed exactly as it would be without the marker.
	laterWithMarker := Attribute(AttributeInput{
		Caps:                CapabilitiesFor(TierProxy),
		Model:               "claude-opus-4-8",
		Now:                 t0,
		CacheCreationTokens: 1234,
	...
	if laterWithMarker.Kind != KindWrite || laterWithMarker.Cause != CauseSuffixGrowth {
		t.Errorf("later+marker: got (kind=%q, cause=%q), want (write, suffix_growth)",
			laterWithMarker.Kind, laterWithMarker.Cause)
	}
}
