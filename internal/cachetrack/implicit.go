...
//      against. Session-handoff variant: when the bootstrap turn
//      carries the `superbased-handoff <id>` marker the cause is
//      CauseHandoffRehydration instead (same KindImplicitWrite) so
//      the advisor + cache-health exempt the by-design rehydration
//      write — mirrors the Anthropic lane's ruleHandoffRehydration.
...
type ImplicitObservation struct {
	...
	// HandoffMarker is true when this turn's content carries the
	// session-handoff marker (`superbased-handoff <id>`), set at
	// the proxy / store boundary via a cheap bytes scan and
	// threaded through the engine (mirrors [AttributeInput.
	// HandoffMarker] on the Anthropic lane). Consumed ONLY by
	// [ruleImplicitHandoffRehydration], which self-gates on the
	// bootstrap condition (!PriorObserved), so the marker
	// persisting into every later turn's history is inert.
	HandoffMarker bool
}
...
//  2. !PriorObserved (bootstrap) && HandoffMarker → KindImplicitWrite
//     + CauseHandoffRehydration (the by-design first-turn cold write
//     of a session-handoff TARGET; same KindImplicitWrite as the
//     plain bootstrap so every kind-based denominator/exclusion is
//     unchanged — only the cause differs)
//  3. !PriorObserved (bootstrap) → KindImplicitWrite + CauseReanchor
//  4. PriorPrefixTokens > 0 && PriorPrefixTokens <
//     MinCacheableImplicit(Model) → KindBelowMin +
//     CauseBelowMinCacheable
//  5. ObservedCachedTokens == 0 && PriorPrefixTokens >=
//     MinCacheableImplicit(Model) → KindImplicitMiss +
//     CausePrefixChurn
//  6. ObservedCachedTokens > 0 && ObservedCachedTokens <
//     PriorPrefixTokens - OpenAIPrefixGranule → KindImplicitHit +
//     CausePrefixShrink (cached less than tracked — prefix shrank)
//  7. ObservedCachedTokens > 0 → KindImplicitHit + CauseImplicitHit
//  8. (fallthrough) ObservedCachedTokens == 0 && PriorPrefixTokens
...
var implicitAttributionRules = []implicitAttributionRule{
	...
	{Cause: CauseHandoffRehydration, Fn: ruleImplicitHandoffRehydration},
...
// ruleImplicitHandoffRehydration — Session Handoff row, placed
// immediately before ruleImplicitReanchor (first match wins) so it
// intercepts the bootstrap turn: the FIRST turn of a handoff TARGET
// session is a large cold implicit-cache write BY DESIGN (the
// rendered handover doc arrives as the injected first prompt, the
// SessionStart context, or an early HANDOFF-<id>.md file read).
// Fires only on the bootstrap turn (!PriorObserved) that carries the
// handoff marker, and emits the SAME KindImplicitWrite as a plain
// bootstrap so every Kind-based denominator/exclusion keeps working
// (all three implicit kinds route to bucketSkipped + isRateSkipped;
// the predicted-kind on a bootstrap turn is also KindImplicitWrite,
// so ImplicitCacheConsistency excludes it from the graded
// denominator either way) — only the cause differs
// (handoff_rehydration), letting the advisor + cache-health exempt
// the by-design write. On every subsequent turn the marker still
// rides in the conversation history but PriorObserved is true, so
// this rule cannot fire — a marker on a non-first turn changes
// nothing. Mirrors [ruleHandoffRehydration] on the Anthropic lane.
func ruleImplicitHandoffRehydration(obs ImplicitObservation) (bool, Kind, Cause) {
	if !obs.PriorObserved && obs.HandoffMarker {
		return true, KindImplicitWrite, CauseHandoffRehydration
	}
	return false, "", ""
}

// ruleImplicitReanchor — Row 3: bootstrap turn for a (session,
...
// ruleImplicitBelowMin — Row 4: tracked prefix below the model's
...
// ruleImplicitPrefixChurn — Row 5: observed cached_tokens = 0 on
...
// ruleImplicitPrefixShrink — Row 6: observed cached_tokens > 0
...
// ruleImplicitHit — Row 7: observed cached_tokens > 0 within the
