...
func TestAttributeImplicit_DecisionTable(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "row 2 — bootstrap turn + handoff marker → handoff_rehydration (same KindImplicitWrite)",
			obs: ImplicitObservation{
				Model:                "gpt-5",
				PriorObserved:        false,
				PriorPrefixTokens:    0,
				ObservedCachedTokens: 8192, // large cold prefix, by design
				HandoffMarker:        true,
			},
			wantKind:  KindImplicitWrite,
			wantCause: CauseHandoffRehydration,
		},
		{
			name: "row 3 — bootstrap turn (no prior, no marker) reanchors",
			obs: ImplicitObservation{
				Model:                "gpt-5",
				PriorObserved:        false,
				PriorPrefixTokens:    0,
		...
		{
			name: "marker on a LATER turn (PriorObserved) is inert — normal hit, not handoff",
			obs: ImplicitObservation{
				Model:                "gpt-5",
				PriorObserved:        true,
				PriorPrefixTokens:    4096,
				ObservedCachedTokens: 4096,
				HandoffMarker:        true, // still riding in history; must NOT re-fire handoff
			},
...
// TestObserveTurn_ImplicitCache_HandoffMarker proves the handoff
// marker threads from ObserveInput through observeImplicit into the
// reduced attribution: a handoff TARGET's bootstrap implicit turn
// attributes as cause=handoff_rehydration (same KindImplicitWrite,
// so denominators are unmoved), and a marker riding into a LATER
// turn is inert (normal implicit_hit, not a second rehydration).
func TestObserveTurn_ImplicitCache_HandoffMarker(t *testing.T) {
	t.Parallel()
	eng := NewEngine(0)
	...
	// Bootstrap turn of a handoff target — large cold write BY DESIGN.
	res := eng.ObserveTurn(ObserveInput{
		SessionID:     "sess-handoff-implicit",
		Model:         "gpt-5",
		Scope:         "default",
		Tier:          TierProxy,
		Caps:          caps,
		...
		Usage: CacheUsageObserved{
			NetInputTokens:  8192,
			OutputTokens:    100,
			CacheReadTokens: 0, // cold on the first turn
		},
		APITurnID: 1,
	})
	if len(res.Events) != 1 {
	...
	if res.Events[0].Outcome.Kind != KindImplicitWrite || res.Events[0].Outcome.Cause != CauseHandoffRehydration {
		t.Errorf("bootstrap+marker event = %q/%q, want implicit_write/handoff_rehydration",
			res.Events[0].Outcome.Kind, res.Events[0].Outcome.Cause)
	}

	// Later turn — marker STILL rides in history, but Prior is now
	// established, so the rule is inert: a normal implicit_hit.
	now2 := now.Add(time.Second)
	res2 := eng.ObserveTurn(ObserveInput{
		SessionID:     "sess-handoff-implicit",
		Model:         "gpt-5",
		...
		MessageID:     "req-2",
		Now:           now2,
		HandoffMarker: true, // persists in conversation history
		Usage: CacheUsageObserved{
			NetInputTokens:  9216,
			OutputTokens:    100,
			CacheReadTokens: 8192, // matches the seeded prefix
		},
		APITurnID: 2,
	})
	if len(res2.Events) != 1 {
		t.Fatalf("later turn: events=%d, want 1", len(res2.Events))
	}
	if got := res2.Events[0].Outcome.Cause; got == CauseHandoffRehydration {
		t.Errorf("marker on a later turn re-fired handoff_rehydration — must be inert, got cause=%q", got)
	}
	if res2.Events[0].Outcome.Kind != KindImplicitHit {
		t.Errorf("later turn kind = %q, want implicit_hit", res2.Events[0].Outcome.Kind)
	}

	// The handoff event carries KindImplicitWrite, so both the §10
	// gate and the implicit-cache consistency metric exclude it —
	// prove the G2/G3 counters never moved.
	if got := eng.Counters().BucketMispredictTotal; got != 0 {
		t.Errorf("BucketMispredictTotal after handoff implicit turns = %d, want 0", got)
	}
}
