...
const (
	...
	// CauseHandoffRehydration — Session Handoff variant of
	// reanchor: the FIRST turn of a handoff TARGET session is a
	// large cold cache write BY DESIGN — the rendered handover
	// doc arrives as the injected first prompt, the SessionStart
	// additionalContext, or an early HANDOFF-<id>.md file read.
	// It carries the SAME KindReanchor as a plain first turn (so
	// every Kind-based denominator/exclusion keeps working), but
	// the distinct cause lets the advisor + cache-health treat it
	// as a baseline (non-waste, non-thrash) event rather than
	// flagging the intentional rehydration write. Fires only when
	// the observed turn carries the `superbased-handoff <id>`
	// marker AND it is the first turn for (session, model, scope);
	// on every subsequent turn the marker still rides in the
	// conversation history but Prior != nil, so the rule is inert.
	CauseHandoffRehydration Cause = "handoff_rehydration"
...
type AttributeInput struct {
	...
	// the proxy / store boundary via a cheap bytes scan. Consumed
	// ONLY by ruleHandoffRehydration, which self-gates on
	// Prior == nil, so the marker persisting into every later
	// turn's history is inert.
	HandoffMarker bool
...
var attributionRules = []attributionRule{
	{Cause: CauseHandoffRehydration, Fn: ruleHandoffRehydration},
...
// ruleHandoffRehydration — Session Handoff row (placed immediately
// before reanchor, first match wins): the target session's FIRST
// turn is a large cold cache write BY DESIGN (the handover doc
// arrives as the injected first prompt / SessionStart context / an
// early HANDOFF-<id>.md file read). Fires only on the first
// observed turn (Prior == nil) that carries the handoff marker,
// and emits the SAME KindReanchor as a plain first turn so every
// Kind-based denominator/exclusion keeps working — only the cause
// differs (handoff_rehydration), so the advisor + cache-health can
// exempt the by-design write. On every subsequent turn the marker
// still rides in the conversation history, but Prior != nil so this
// rule cannot fire — a marker on a non-first turn changes nothing.
func ruleHandoffRehydration(in AttributeInput) (bool, Kind, int, BlockLevel) {
	if in.Prior == nil && in.HandoffMarker {
