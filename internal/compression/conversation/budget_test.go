...
// TestEnforceSkipDropsCapability pins the provider-neutral no-drop
// capability (BudgetOptions.SkipDrops, fed by
// [compression.conversation].disable_drops). SkipDrops must eliminate
// Pass-2 eviction while leaving full per-type compression eligibility
// intact — the fix for the 2026-07-16 drop-only degeneration on the
// OpenAI/codex lane where per-type compressors no-op on codex tool
// outputs.
func TestEnforceSkipDropsCapability(t *testing.T) {
	// Incompressible, non-preserved messages that would ordinarily be
	// dropped at a low target ratio. Plain repeated bytes classify as
	// text (not in the allow list below), so per-type compression can't
	// help and only the drop pass could hit target.
	incompressible := func() []ScoredMessage {
		return []ScoredMessage{
			scoredMsg(RoleUser, "prompt", 0.9, true),
			scoredMsg(RoleTool, strings.Repeat("A", 400), 0.1, false),
			scoredMsg(RoleTool, strings.Repeat("B", 400), 0.15, false),
			scoredMsg(RoleTool, strings.Repeat("C", 400), 0.2, false),
		}
	}

	// A compressible JSON tool_result alongside incompressible filler,
	// to prove per-type compression STILL runs under SkipDrops.
	bigJSON := `{"records":[`
	for i := 0; i < 80; i++ {
		if i > 0 {
			bigJSON += ","
	...
	tests := []struct {
		...
		{
			name: "back-compat drops when SkipDrops off",
			msgs: incompressible(),
			opts: BudgetOptions{
				TargetRatio:   0.3,
				Registry:      DefaultRegistry(),
		...
		{
			name: "SkipDrops eliminates eviction on incompressible body",
			msgs: incompressible(),
			opts: BudgetOptions{
				TargetRatio:   0.3,
				Registry:      DefaultRegistry(),
		...
		{
			name: "SkipDrops keeps per-type compression",
			msgs: []ScoredMessage{
				scoredMsg(RoleUser, "prompt", 0.9, true),
				scoredMsg(RoleTool, bigJSON, 0.1, false),
				scoredMsg(RoleTool, strings.Repeat("A", 400), 0.15, false),
			},
			opts: BudgetOptions{
				TargetRatio:   0.3,
				Registry:      DefaultRegistry(),
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			n := len(tc.msgs)
			got := Enforce(tc.msgs, tc.opts)
			if tc.wantDropped && got.Stats.DroppedCount == 0 {
				t.Errorf("expected drops, got none: %+v", got.Stats)
			}
			if !tc.wantDropped && got.Stats.DroppedCount != 0 {
				t.Errorf("expected zero drops, got %d: %+v", got.Stats.DroppedCount, got.Stats)
			}
			if tc.wantCompress && got.Stats.CompressedCount == 0 {
				t.Errorf("expected per-type compression to fire, got none: %+v", got.Stats)
			}
			if tc.wantAllSurvive {
				if len(got.Messages) != n {
					t.Errorf("message count = %d, want %d (nothing dropped)", len(got.Messages), n)
				}
				for _, m := range got.Messages {
					if strings.Contains(m.Text, "messages compressed") {
						t.Errorf("unexpected drop marker under SkipDrops: %q", trimPreview(m.Text))
					}
				}
			}
			// No 'drop' events must ever be recorded when SkipDrops is set.
			if tc.opts.SkipDrops {
				for _, ev := range got.Stats.Events {
					if ev.Mechanism == "drop" {
						t.Errorf("SkipDrops recorded a drop event: %+v", ev)
					}
				}
			}
		})
