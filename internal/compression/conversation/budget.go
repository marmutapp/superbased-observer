...
type BudgetOptions struct {
	...
	CacheAware bool
	// SkipDrops disables Pass 2 (lossy eviction) WITHOUT the other
	// cache-aware narrowing that CacheAware implies. Per-type
	// compression eligibility stays full (every non-preserved,
	// non-NoCompress message per its role rules), so legitimate
	// content-preserving compression is unaffected; only the drop pass
	// is skipped. When the target ratio is then unreachable (the
	// per-type compressors no-op), Enforce fails open: it returns
	// whatever Pass-1 compression achieved — even nothing — never
	// dropping or truncating. This is the provider-neutral no-drop
	// capability threaded from [compression.conversation].disable_drops;
	// CacheAware also skips drops but is Anthropic-cache-shaped (narrows
	// per-type to RoleTool). Either flag being set skips Pass 2.
	SkipDrops bool
}
...
func Enforce(scored []ScoredMessage, opts BudgetOptions) BudgetResult {
	...
	// No-drop paths: cache-aware mode never drops (drop ranking is
	// budget-relative and would shift across turns, invalidating the
	// prefix cache); SkipDrops is the provider-neutral no-drop capability
	// (disable_drops) that fails open when the target is unreachable
	// rather than degenerating into drop-only eviction. Either skips
	// Pass 2 and returns whatever per-type compression achieved.
	if opts.CacheAware || opts.SkipDrops || stats.CompressedBytes <= target {
		return BudgetResult{
