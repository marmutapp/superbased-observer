...
// forwarded byte-identically (fast-path taken). This is the default
// behaviour for every shipped recipe — claude-code (["json","logs",
// "code"]), codex-safe (["logs"]), and codex-variant ([]) all omit
// "tools" — so this test pins the BC blast radius described in
