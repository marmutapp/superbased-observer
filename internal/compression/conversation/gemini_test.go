...
// geminiDisableDropsPipeline mirrors geminiDropPipeline (json-only per-type
// compression, aggressive ratio) but sets DisableDrops so the Gemini drop
// pass is gated off — isolating the disable_drops behaviour under test.
func geminiDisableDropsPipeline(ratio float64, preserveLastN int) *Pipeline {
	return NewPipeline(PipelineConfig{
		Enabled:       true,
		TargetRatio:   ratio,
		PreserveLastN: preserveLastN,
...
// TestRunGoogle_DisableDropsSkipsEviction pins that the Google lane honours
// disable_drops with the SAME semantics as the OpenAI/Anthropic lanes: an
// over-budget conversation passes through VALID with zero drop events
// (fail-open toward a larger-but-valid body), and every round-trip survives.
// The knob-off arm confirms the same conversation still drops when the knob
// is not set (behaviour unchanged).
func TestRunGoogle_DisableDropsSkipsEviction(t *testing.T) {
	body := sevenTurnBody(t)

	// Knob ON: under ratio 0.1 the drop pass would normally evict the alpha
	// and beta round-trips; disable_drops must skip that eviction entirely.
	on := geminiDisableDropsPipeline(0.1, 1).Run("google", body)
	if !json.Valid(on.Body) {
		t.Fatalf("disable_drops body is not valid JSON (would 400): %s", on.Body)
	}
	if on.DroppedCount != 0 {
		t.Errorf("DroppedCount: got %d want 0 (disable_drops must skip eviction)", on.DroppedCount)
	}
	for _, e := range on.Events {
		if e.Mechanism == "drop" {
			t.Errorf("unexpected drop event under disable_drops: %+v", e)
		}
	}
	s := string(on.Body)
	if !strings.Contains(s, "alpha") || !strings.Contains(s, "beta") {
		t.Errorf("disable_drops must preserve all round-trips; alpha/beta missing: %s", s)
	}
	assertGeminiAlternating(t, geminiResultRoles(t, on.Body))

	// Knob OFF: same conversation still drops (Google-lane default unchanged).
	off := geminiDropPipeline(0.1, 1).Run("google", body)
	if off.DroppedCount == 0 {
		t.Fatalf("knob-off Google lane should still drop, got DroppedCount=0")
	}
}
