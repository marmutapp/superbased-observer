...
func Build(ctx context.Context, deps Deps, req Request) (Result, error) {
	...
	transcript, degrade := readTranscript(ctx, deps, sub)
	...
	if delivery == integration.InjectHook {
		...
		hookExpires = now().UTC().Add(time.Duration(ttl) * time.Minute)
...
func readTranscript(ctx context.Context, deps Deps, sub store.HandoffSubstrate) ([]models.TranscriptMessage, string) {
	return readSessionTranscript(ctx, deps.Adapters, sub.Session, sub.SourceFiles)
}
...
func readSessionTranscript(ctx context.Context, adapters []adapter.Adapter, sess models.Session, sourceHints []string) ([]models.TranscriptMessage, string) {
	cap, _ := integration.For(sess.Tool)
	if cap.Handoff.Transcript == integration.TranscriptActionsOnly {
		return nil, fmt.Sprintf("%s has no readable transcript capability — metadata carry", sess.Tool)
	}
	...
	if reader == nil {
		return nil, fmt.Sprintf("%s transcript reader not implemented yet (classified %s) — metadata carry", sess.Tool, cap.Handoff.Transcript)
	}
	msgs, err := reader.ReadTranscript(ctx, sess, sourceHints)
	if err != nil {
		return nil, fmt.Sprintf("transcript unreadable (%v) — metadata carry", err)
	}
	if len(msgs) == 0 {
		return nil, "transcript on disk holds no messages — metadata carry"
	}
	return msgs, ""
}
