...
func TestBuild_ResolvesResultProjectRoot(t *testing.T) {
	// A project root already reachable from this OS is unchanged
	// (translation is idempotent) and is exposed on the Result, written
	// into as the doc directory, and recorded on the stored row — the one
	// resolved value every consumer agrees on.
	root := t.TempDir()
	deps, fs := testDeps(t, root)
	res, err := Build(context.Background(), deps, Request{SessionID: "s1", TargetTool: "codex"})
	if err != nil {
	...
	if res.ProjectRoot != root {
		t.Errorf("result project root = %q, want %q", res.ProjectRoot, root)
	}
	if !strings.HasPrefix(res.DocPath, root) {
		t.Errorf("doc path = %q, want under %q", res.DocPath, root)
	}
	if len(fs.inserted) != 1 || fs.inserted[0].ProjectRoot != root {
		t.Errorf("stored row project root = %q, want %q", fs.inserted[0].ProjectRoot, root)
	}
}

func TestBuild_TranslatesForeignProjectRoot(t *testing.T) {
	// A Windows-run source tool correctly records its OWN project root in
	// Windows convention (`C:\proj`). This handoff runs under WSL, so Build
	// must translate it to the reachable `/mnt/c/proj` before it becomes the
	// doc directory and the launch cwd — otherwise the doc (and any
	// continuation) land in the daemon's own folder, not the project.
	// DryRun so the assertion holds without a live /mnt mount to write into.
	deps, _ := testDeps(t, `C:\programsx\regulation`)
	res, err := Build(context.Background(), deps, Request{
		SessionID:  "s1",
		TargetTool: "codex",
		DryRun:     true,
	...
	const want = "/mnt/c/programsx/regulation"
	if res.ProjectRoot != want {
		t.Errorf("resolved project root = %q, want %q", res.ProjectRoot, want)
	}
}
