...
func (t *translator) handle(ev kernelEvent) []processobs.RawEvent {
	...
	case evConnect:
		return []processobs.RawEvent{{
			...
			ExePath:         ev.Comm,
			NetworkProtocol: protocolName(ev.Protocol),
			NetworkFamily:   familyName(ev.Family),
			RemoteAddr:      ev.RemoteAddr,
			RemotePort:      int(ev.RemotePort),
			LocalAddr:       ev.LocalAddr,
			LocalPort:       int(ev.LocalPort),
			NetworkStatus:   networkStatus(ev.Status),
			NetworkBytesIn:  0,
			NetworkBytesOut: 0,
		}}
	default:
...
func networkStatus(status int32) string {
	if status == 0 {
		return "ok"
	}
	return "errno:" + strconv.FormatInt(int64(status), 10)
}
