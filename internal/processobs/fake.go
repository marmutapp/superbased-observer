...
// SliceEventSink records high-signal process events in memory for tests.
type SliceEventSink struct {
	Events []ProcessEvent
	Err    error
}

// PersistProcessEvents implements EventSink.
func (s *SliceEventSink) PersistProcessEvents(_ context.Context, events []ProcessEvent) (int, error) {
	if s.Err != nil {
		return 0, s.Err
	}
