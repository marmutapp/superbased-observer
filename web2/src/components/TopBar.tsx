...
import { useCrumbValue } from "@/lib/crumb";
...
// sectionFor returns the matched breadcrumb section (its root path + label) for
// a pathname, so a detail route can render its section as a link back to the
// list. Longest-prefix wins (TITLES is ordered specific→general).
function sectionFor(pathname: string): { prefix: string; label: string } {
...
export function TopBar({ onHelp }: { onHelp?: () => void }) {
  ...
  // A detail route publishes its entity name via the crumb context; when set
  // (and we're deeper than the section root) the section becomes a link and the
  // entity name is the leaf crumb.
  const detail = useCrumbValue();
  const onDetail = detail != null && pathname !== section.prefix;
  ...
  return (
    <header className="flex h-[var(--header-h)] items-center justify-between gap-3 border-b border-line-1 bg-bg-1 px-5">
      <div className="flex items-center gap-2 text-[12px] text-fg-3">
        ...
        {onDetail ? (
          <>
            <Link to={section.prefix} className="hover:text-fg-1">
              {section.label}
            </Link>
            <span>/</span>
            <b className="max-w-[280px] truncate text-fg-1" title={detail ?? ""}>
              {detail}
            </b>
          </>
        ) : (
          <b className="text-fg-1">{section.label}</b>
        )}
      </div>
