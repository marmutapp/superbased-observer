...
// elevated surface). Grouping makes the governance vs admin split legible as
// the nav grows. Nav-count badges (Teams / People / Projects / Sessions) are
// fed cheaply from the existing /api/org/overview rollup (window-scoped counts)
// — no dedicated status endpoint needed. Counts hide at zero so a fresh org
// stays quiet.
...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: "/trajectories/end-users", label: "End-user spend", icon: UserRound },
      { to: "/trajectories/evals", label: "Eval health", icon: FlaskConical },
      { to: "/trajectories/alerts", label: "Alerts", icon: BellRing },
      { to: "/trajectories/admission", label: "Admission", icon: ShieldCheck },
    ],
  },
  {
    id: "optimize",
...
export function Sidebar() {
  ...
  // Single cheap read feeds every badge; degrades silently (no data → no
  // badges). Window-scoped, matching the counts the pages themselves show.
  const { data } = useApi(() => api.overview(days), [days]);
  const counts: Record<string, number | undefined> = {
    "/teams": data?.team_count,
    "/people": data?.active_developers,
            ...
            {g.items.map((it) => {
              ...
              return (
                ...
                >
                  ...
                  {count != null && count > 0 && (
                    <span className="shrink-0 rounded-pill bg-bg-3 px-1.5 py-px font-mono text-[10px] text-fg-3">
                      {compact(count)}
                    </span>
                  )}
                </NavLink>
