...
export function SessionsPage() {
  ...
  // Deep-link support: the command palette navigates here with ?focus=<id> to
  // open a specific session's detail drawer. Consume it once on mount, then
  // strip it so a manual close doesn't reopen on refresh.
  ...
  return (
    <>
      ...
      {error ? (
        ...
        tool || model ? (
          <Empty message="No sessions match the filters." />
        ) : (
          <EmptyState
            icon={History}
            title="No sessions in scope for this window"
            body="Sessions roll up here as enrolled nodes push their activity. If you expected data, widen the window (top-right), or enroll an agent so its sessions reach the server. This list is scoped to your role."
            helpId="tab.sessions"
            to="/invite"
            toLabel="Enroll an agent"
          />
        )
      ) : (
        <Card className="p-3">
