...
export const HELP_REGISTRY: HelpEntry[] = [
  ...
  },
  {
    ...
    oneLiner:
      "Cross-instance spend attributed to your hosted app's end-users — a per-customer view. Admin-only, PII-gated.",
    detail:
      "The T5 obs_enduser_spend surface: spend attributed to the hosted-app's END-USER identity (distinct from Trajectory cost, which attributes to the developer/project/model), summed across every enrolled node that observed that end-user. Because an end-user id is PII, a node ships it ONLY under [org_client.share].obs_summary AND raw-content sharing (full_content, or admin_managed under a native-console deployment) — there is no remote toggle. Shows an honest empty state naming both dependencies when nothing has been shared.",
    related: ["tab.obs_cost", "tile.obs.endusers", "tile.obs.enduser_cost", "glossary.content_free"],
  },
  {
    id: "tab.obs_evals",
    category: "tab",
  ...
  },
  {
    ...
    oneLiner:
      "One run's per-item scores, a run-vs-run per-scorer diff, and an audited item-content read. Admin-only.",
    detail:
      "The T7 obs_eval_items surface: per-item span/scorer/score/pass rows nodes push under [org_client.share.obs].eval_items (default off) — content-free, so the item body is never shipped by default. Pick another run of the same dataset for per-scorer Base/Compare/Δ. The item input/expected/output excerpts require full_content and are a separate, server-audited read (a view_eval_item_content audit row is written before the excerpts return).",
    related: ["tab.obs_evals", "tile.obs.pass_rate", "glossary.eval_run", "glossary.content_free"],
  },
  {
    id: "tab.obs_alerts",
    category: "tab",
  ...
  {
    id: "tab.obs_admission",
    category: "tab",
    title: "Admission",
    oneLiner:
      "Input-admission verdicts + policy versions enrolled nodes shared — an admin gate over incoming app requests. Monitoring-only, content-free by default.",
    detail:
      "The Plane-A admission surface: the verdict metadata (decision / severity / criterion / judge / latency), policy version history, and would-block counts a node shares under [org_client.share.obs].admission (default off) — never the request body. Authoring stays node-side (observer obs admission setup / config.toml); there is deliberately NO remote apply here, matching the never-server-forced posture. The human-readable reason is a separate, server-audited lazy read; per-end-user would-block/spend appear only where a node also shares full content.",
    related: [
      "tile.obs.admission_mode",
      "tile.obs.admission_judge",
      "tile.obs.admission_verdicts",
      "tile.obs.admission_chain",
      "glossary.audited_disclosure",
      "glossary.content_free",
    ],
  },
  {
    id: "tile.obs.admission_mode",
    category: "tile",
    title: "Admission mode tile",
    oneLiner: "The latest shared policy mode — observe (shadow) or enforce (blocking). Highlights on enforce.",
    detail:
      "Admission mode is a node-side choice. observe records verdicts without blocking; enforce rejects would-block requests at the node. The tile highlights when a node is enforcing. There is no remote enforce toggle — the org cannot flip a node into enforce.",
    related: ["tab.obs_admission", "tile.obs.admission_verdicts"],
  },
  {
    id: "tile.obs.admission_judge",
    category: "tile",
    title: "Admission judge tile",
    oneLiner: "Where the admission LLM judge runs — local (Ollama/self-hosted), remote, or pre-filters only.",
    detail:
      "Content-based criteria are judged by an LLM the node hosts (local), a remote/gateway model, or not at all (pre-filters only). The hosting label is node-declared; the judged content itself never reaches this server.",
    related: ["tab.obs_admission"],
  },
  {
    id: "tile.obs.admission_verdicts",
    category: "tile",
    title: "Verdicts 24h tile",
    oneLiner: "Admission verdicts in the last 24h — allow + flag + would-block, with the sub-breakdown.",
    detail:
      "The count of recorded verdicts over the trailing 24h, split allow / flag / would-block. Highlights when any request would have been blocked. Content-free — a verdict is a decision + criterion, never the request body.",
    related: ["tab.obs_admission", "tile.obs.admission_mode"],
  },
  {
    id: "tile.obs.admission_chain",
    category: "tile",
    title: "Audit chain tile",
    oneLiner: "Integrity of the tamper-evident verdict hash-chain — intact / BROKEN, or N/M nodes intact across nodes.",
    detail:
      "Admission verdicts are recorded in a hash-chained audit log so a deleted or altered verdict is detectable. The tile reports whether the shared chain verifies; with multiple enrolled nodes it shows how many nodes' chains verified. A BROKEN chain warrants investigation.",
    related: ["tab.obs_admission", "glossary.audited_disclosure"],
  },
  {
    id: "tile.obs.traces",
    category: "tile",
  ...
  {
    id: "tile.obs.enduser_cost",
    category: "tile",
    title: "End-user total spend tile",
    oneLiner: "Total cross-instance spend attributed to your app's end-users in the window.",
    detail:
      "Sum of per-end-user cost over the window, aggregated across every enrolled node that observed each end-user (obs_enduser_spend carries one row per node × day × end-user, so summing across nodes yields the cross-instance total).",
    related: ["tab.obs_endusers", "tile.obs.endusers", "glossary.content_free"],
  },
  {
    id: "tile.obs.endusers",
    category: "tile",
    title: "End-users tile",
    oneLiner: "Distinct hosted-app end-users spend is attributed to.",
    detail:
      "Count of end-user identities the shared spend splits across in the window. The end-user id is PII, shared only under the obs_summary opt-in plus raw-content sharing.",
    related: ["tab.obs_endusers", "tile.obs.enduser_cost"],
  },
  {
