...
// Org per-item eval detail (obs-org-tier T7). Mirrors rollup.ObsEval* (obseval.go).
// Distinct from the T4 aggregate above: these carry per-item scores that let
// the org Evals page drill into one run and diff two runs cell-for-cell.
export interface ObsEvalRunListRow {
  ref: string; // opaque (node, run_id) handle for the detail/compare endpoints
  user_email: string; // the pushing operator (run author)
  run_name: string;
  dataset_name: string;
  items: number;
  scores: number;
...
export interface ObsEvalRunsResult {
  window_days: number;
  configured: boolean;
  runs: ObsEvalRunListRow[];
}
export interface ObsEvalItemScoreRow {
  item_id: number;
  span_id: string;
  trace_id: string;
  scorer: string;
...
export interface ObsEvalRunDetailResult {
  window_days: number;
  run: ObsEvalRunListRow;
  scores: ObsEvalItemScoreRow[];
}
export interface ObsEvalScorerDelta {
  scorer: string;
  base_count: number;
  ...
  compare_pass_rate: number;
  pass_rate_delta: number;
}
export interface ObsEvalCompareResult {
  base: ObsEvalRunListRow;
  compare: ObsEvalRunListRow;
  scorers: ObsEvalScorerDelta[];
}
export interface ObsEvalItemContentRow {
  item_id: number;
  span_id: string;
  scorer: string;
  ts: string;
...
export interface ObsEvalItemContentResult {
  items: ObsEvalItemContentRow[]; // AUDITED server-side; empty when nodes don't share content
}
...
// Mirrors rollup.ObsEndUserSpend*. DISTINCT from ObsCost (which attributes to
// the DEVELOPER/project/model): this attributes CROSS-INSTANCE spend to the
// hosted-app's end-user identity — PII, so it rides the obs_summary opt-in AND
// node-side raw-content sharing. Admin-only.
export interface ObsEndUserBucket {
  end_user: string;
  cost_usd: number;
  traces: number;
  total_tokens: number;
...
export interface ObsEndUserSpendResult {
  window_days: number;
  configured: boolean;
  total_cost_usd: number;
  users: ObsEndUserBucket[];
}
// Org observability input-admission monitoring (Plane-A admission, gap-audit
// #1b). READ-ONLY org view over the admission verdicts + policy versions
// enrolled nodes share under [org_client.share.obs].admission. Content-free by
// default: verdicts carry decision/severity/criterion metadata, never the
// request body; end_user is populated only where a node also shares full
// content, and the human-readable REASON never rides this result (it is a
// separate, server-audited read). Authoring is node-side only — the org
// surface is monitoring-only, there is deliberately NO remote policy write.
// Admin-only. Mirrors rollup.ObsAdmission*.
export interface ObsAdmissionVerdictCounts {
  allow: number;
  flag: number;
  would_block: number;
}
export interface ObsAdmissionChainNode {
  user_email: string;
  rows: number;
  ok: boolean;
}
export interface ObsAdmissionChain {
  ok: boolean;
  nodes: ObsAdmissionChainNode[];
}
export interface ObsAdmissionPolicyVersion {
  policy_hash: string;
  user_email: string;
  created_at: string;
  mode: string;
...
export interface ObsAdmissionVerdictRow {
  ts: string;
  user_email: string;
  mode: string;
  decision: string;
...
export interface ObsAdmissionUserBlocks {
  end_user: string;
  would_block: number;
  flag: number;
}
export interface ObsAdmissionResult {
  window_days: number;
  configured: boolean;
  mode: string; // latest node policy mode: observe | enforce | ""
  judge_hosting: string; // e.g. local | remote | ""
  criteria_count: number;
  verdicts_24h: ObsAdmissionVerdictCounts;
  chain: ObsAdmissionChain;
  policies: ObsAdmissionPolicyVersion[];
  verdicts: ObsAdmissionVerdictRow[];
  would_block_by_user: ObsAdmissionUserBlocks[]; // empty unless full-content nodes
}
export interface ObsAdmissionReason {
  ts: string;
  request_id: string;
  reason_excerpt: string;
}
export interface ObsAdmissionReasonsResult {
  reasons: ObsAdmissionReason[]; // AUDITED server-side; may be empty when nodes don't share content
}
...
export const api = {
  ...
  // Org per-item eval detail (obs-org-tier T7) — admin-only.
  obsEvalRuns: (days?: number) => request<ObsEvalRunsResult>(withDays("/api/org/obs/eval/runs", days)),
  obsEvalRun: (ref: string, days?: number) =>
    request<ObsEvalRunDetailResult>(
      `/api/org/obs/eval/run?id=${encodeURIComponent(ref)}${days ? `&days=${days}` : ""}`,
    ),
  obsEvalCompare: (base: string, compare: string) =>
    request<ObsEvalCompareResult>(
      `/api/org/obs/eval/compare?base=${encodeURIComponent(base)}&compare=${encodeURIComponent(compare)}`,
    ),
  // Per-item eval content excerpts — a SEPARATE, server-AUDITED disclosure. Call
  // ONLY on an explicit user action (the server writes an audit row per read).
  obsEvalRunContent: (ref: string) =>
    request<ObsEvalItemContentResult>(`/api/org/obs/eval/run/content?id=${encodeURIComponent(ref)}`),
  ...
  // Org observability per-END-USER spend (T5) — admin-only (SAML), cross-instance.
  obsEndUserSpend: (days?: number) =>
    request<ObsEndUserSpendResult>(withDays("/api/org/obs/enduser-spend", days)),
  // Org input-admission monitoring (Plane-A admission) — admin-only, READ-ONLY.
  obsAdmission: (days?: number) =>
    request<ObsAdmissionResult>(withDays("/api/org/obs/admission", days)),
  // Admission reason excerpts — a SEPARATE, server-AUDITED disclosure. Call
  // ONLY on an explicit user action (the server writes an audit row per read).
  obsAdmissionReasons: (days?: number) =>
    request<ObsAdmissionReasonsResult>(withDays("/api/org/obs/admission/reasons", days)),
