...
export class DaemonManager implements vscode.Disposable {
  ...
  private async spawnDaemon(): Promise<void> {
    ...
    // A failed spawn must ABORT the readiness wait below. Without this the
    // 'error' handler would mark the daemon idle but spawnDaemon would still
    // grind through the full withBackoff retry schedule and then fire a second,
    // misleading "dashboard did not respond" toast on top of the accurate
    // spawn-error toast. We race the health probe against spawnFailure (which
    // the error handler rejects) and suppress the health-probe toast when
    // spawnErrored is set.
    ...
    // The health probe usually wins the race, leaving spawnFailure unsettled —
    // attach a no-op catch so that never surfaces as an unhandled rejection.
    spawnFailure.catch(() => {});
    // Without an 'error' listener a failed spawn (EACCES on a non-executable
    // binary, ENOEXEC on a corrupt one) becomes an unhandled 'error' event and
    // takes the extension host down. Surface it in the Output channel + a
    // toast, mark the daemon idle, and do NOT trigger crash-recovery: retrying
    // the same unspawnable binary would just loop.
    child.once('error', (err: Error) => {
      output.appendLine(`[daemon!] failed to spawn ${this.binary.path}: ${err.message}`);
    ...
    try {
      await Promise.race([withBackoff(() => this.client.health()), spawnFailure]);
      output.appendLine(`Daemon dashboard ready at ${this.client.url('/')}`);
      this.restartAttempt = 0;
    } catch (err) {
      // If the spawn itself failed, the 'error' handler above already surfaced
      // the accurate toast and marked the daemon idle; returning here keeps
      // that the ONLY user-facing signal and avoids the full retry delay.
