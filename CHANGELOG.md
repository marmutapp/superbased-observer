...
- **Browser-chat capture (opt-in MV3 extension).** Full-detail capture of
  ...
  Load-unpacked only for now. See [`docs/browser-extension.md`](docs/browser-extension.md).
...
- **Terminal Workspace dock grid.** The Terminals page is now a workspace:
  ...
  window**. Layouts persist server-side (node-local `workspace_layouts`,
...
- **Remote dashboard + terminal control (Phase 4, explicit opt-in).** Drive a
  ...
  internet. See [`docs/remote-access.md`](docs/remote-access.md).
- **Process-network body capture + attribution wave.** For observed processes
  ...
  [`docs/process-observability.md`](docs/process-observability.md).
...
- **Shareable cost card.** `observer report share [--period week|month]
  [--markdown]` renders a 1200×630 SVG social card + markdown summary of your
...
- **Website.** Text-first, fully indexable homepage (the Observer Quest
  ...
  PDF; 28 per-tool "track <tool> costs" pages generated from the adapter
...
- **`qwen-code` promoted to a live, observer-driven proxy route.** qwen-code
  ...
  `modelProviders` entry on the known default host** to the proxy URL, keeping
...
- **GPT-5.6 cache/effort grounding closed against live traffic.** A live
  ...
  wire** — only `cached_input_tokens` (read discount). OpenAI docs confirm the
...
- **Codex reasoning tokens were double-billed.** The OpenAI/Codex wire reports
  **GROSS** `output_tokens` with reasoning as a **subset** (`input + output ==
  total` on every live `token_count` event — arithmetic-verified on gpt-5.6
...
- **Seeded session-handoff launchers for the new adapters** — `observer qwen`,
...
- **Guarded proxy-route config-lane writers** — `internal/proxyroute` gained
...
- **Session Handoff (continue-anywhere)** — `observer handoff <session-id>
  [--to <tool>]` distills any captured session from the source tool's OWN
...
- **The node/developer dashboard is now purely coding-agent (Plane B).** The
  ...
  **Admission** policy editor / per-end-user budgets — were removed from the
  ...
  The obs backend, `/api/obs/*` endpoints, and `observer obs …` / `observer
  eval` CLIs are unchanged. See the new [`docs/deployment-models.md`].
...
- **Config auto-migration rail.** Deprecated config keys are now rewritten
  ...
  `config.toml.bak` backup is written first) and on demand via **`observer
  config migrate`** / **`observer config migrate --dry-run`**. Idempotent and
