...
```bash
# Anthropic / Claude Code (no change vs. shipped default behaviour)
observer start --recipe claude-code

# gpt-mini / gpt-full family on codex CLI (gpt-5.4-mini, gpt-5.5, etc.)
observer start --recipe codex-safe

# `*-codex` reasoning family (gpt-5.3-codex, gpt-5.4-codex-high, ...)
...
| OpenAI mini / full (gpt-5.4-mini, gpt-5.5, gpt-5, ...) | `codex-safe` | logs-only + conservative budget |
...
* `compress_types = ["json", "logs", "code"]` — every content-
  preserving compressor in the registry.
...
* `compress_types = ["logs"]` — JSON sentinel substitution destroys
  data values codex relies on, so JSON compression is off. Text
  head-tail is also off. Code skeleton is debatable on codex; we
  leave it off for safety.
