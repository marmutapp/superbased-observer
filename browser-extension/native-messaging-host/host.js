...
//   observer browser hook capture
// with the payload JSON on the child's STDIN — the same landing point the
// loopback listener funnels into. It then replies {status:"ok"} (framed) so
// the extension's sendNativeMessage callback resolves.
//
...
function ingest(payload) {
  const args = ["browser", "hook", "capture"];
