...
// loopback listener funnels into. The <event> is "health" for a health beacon
// (payload.type === "health") and "capture" for every other message. It then
// replies {status:"ok"} (framed) so the extension's sendNativeMessage
// callback resolves.
//
// NOTE: internal/browserhost/hostfiles/host.js is the SHIPPED copy (embedded
// + written by `observer init --browser`); keep this reference copy in sync.
//
// The observer binary is resolved from the OBSERVER_BIN env var (set in the
// host manifest or the environment); it defaults to "observer" on PATH. An
