...
- [MCP server — 19 cross-tool intelligence calls](#mcp-server--19-cross-tool-intelligence-calls)
...
(message-summary LLM, codeintel MCP, Teams org-server). Full details:
...
real-time action feed), Trajectories + Evals (OTLP trace/span capture
for your own agents), Search (full-text over captured tool outputs),
...
### Trajectories & Evals — trace your own agents

<p align="center">
  <img src="docs/assets/screenshots/13-trajectories.png" alt="Trajectories tab — OTLP trace and span capture" width="900">
</p>

Point any OpenTelemetry-instrumented app or agent at the local OTLP
endpoint and its trace/span graph lands here — enriched with the
proxy's **exact** cost and cache numbers wherever a span matches a
proxied turn (the join no external observability tool can make). The
**Evals** tab tracks eval-run health and flags regressions across
runs. Everything stays local and node-opt-in.
...
## MCP server — 19 cross-tool intelligence calls
...
| `get_symbols` | Resolve symbol name + range to file path + body (codeintel-backed). |
