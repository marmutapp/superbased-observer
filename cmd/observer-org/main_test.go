...
func TestMigrateAndDumpConfig(t *testing.T) {
	...
	// Schema 11 = the guard arc's paired server migrations (004 policy
	...
	// Copilot instance #3), plus 011_otel_content_session_idx (Phase 7
	// message-content viewer index; server-only, no agent pair).
	if !strings.Contains(out, "schema version = 11") {
