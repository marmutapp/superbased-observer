...
func TestMigrateAndDumpConfig(t *testing.T) {
	...
	// Schema 10 = the guard arc's paired server migrations (004 policy
	// bundles at G13, 005 guard_events at G14), the model-routing arc's
	// 006_routing, plus the native-console servers: 007_otel_content (Phase
	// 2b Layer B), 008_cc_analytics_daily (Claude Code Analytics poller),
	// 009_codex_analytics_daily (Codex instance #2), and
	// 010_copilot_analytics_daily (Copilot instance #3).
	if !strings.Contains(out, "schema version = 10") {
