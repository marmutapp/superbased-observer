...
func scrubCounts(ctx context.Context, db *sql.DB, selected []scrubColumn) ([]scrubCount, error) {
	...
	for _, sc := range selected {
		...
		// scrub-candidates).
		q := fmt.Sprintf(`SELECT COUNT(*) FROM %s WHERE %s IS NOT NULL AND %s != ''`, sc.table, sc.column, sc.column)
		...
		qt := fmt.Sprintf(`SELECT COUNT(*) FROM %s`, sc.table)
...
func scrubApply(ctx context.Context, db *sql.DB, selected []scrubColumn) error {
	...
	for _, sc := range selected {
		...
		q := fmt.Sprintf(`UPDATE %s SET %s = %s WHERE %s IS NOT NULL AND %s != ''`,
...
// scrubConfigDefaultPath is shadowed for testability — kept as a package
// var so a test can override before invoking newRootCmd().
var scrubConfigDefaultPath = config.DefaultPath
