...
actually rewind.`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			toUnlock := maxIDs.Sessions + maxIDs.Actions + maxIDs.APITurns + maxIDs.TokenUsage
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if res.Empty {
				...
				if cerr == nil && mErr == nil {
					fmt.Fprintf(out, "  cursor:   sessions=%d actions=%d api_turns=%d token_usage=%d\n",
						cur.Sessions, cur.Actions, cur.APITurns, cur.TokenUsage)
					fmt.Fprintf(out, "  max ids:  sessions=%d actions=%d api_turns=%d token_usage=%d\n",
						maxIDs.Sessions, maxIDs.Actions, maxIDs.APITurns, maxIDs.TokenUsage)
				}
				if lpErr == nil && lp != nil {
					fmt.Fprintf(out, "  last push: %s — %s, %d rows, %d bytes%s\n",
						lp.PushedAt, lp.Status, lp.RowCount, lp.Bytes, errSuffix(lp.Error))
