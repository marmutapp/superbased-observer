...
func newAntigravityCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runAntigravityLaunch(bin, args, continueDir)
		},
	}
...
// runAntigravityLaunch execs `agy` NON-PROXIED (child inherits os.Environ()
// with no base-URL redirect), forwarding stdio and the exit code — the same
// shape as `observer run`. continueDir ("" inherits the caller's cwd) is set
// by --continue-from to the source session's translated project root so a
// cross-OS continuation lands in the real project folder.
func runAntigravityLaunch(bin string, args []string, dir string) error {
	...
	if err := child.Run(); err != nil {
		...
		return fmt.Errorf("exec agy: %w", err)
	}
	return nil
}
