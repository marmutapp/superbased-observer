...
// (cmd/observer/launch_dashboard.go, wired in newLaunchManager) is the primary,
...
func (h *attachHost) LaunchAttachable(ctx context.Context, req attachsock.SpawnRequest) (attachsock.Session, error) {
	...
	// untrusted input. Validate it against the integration registry — the ONE
	// owner of adapter capabilities — BEFORE spawning anything. The tool must
	// declare an Attach capability whose Subcommand matches the requested one,
	// so a caller can't drive the daemon to launch an arbitrary `observer <x>`
	// verb the operator never grounded as attachable. termsvc stays registry-
	// free; this check lives at the daemon boundary (CLAUDE.md #3 — branch on
	// the capability shape, not the tool name).
	capab, ok := integration.For(req.Tool)
	if !ok || capab.Attach == nil {
		return nil, fmt.Errorf("attach: tool %q has no grounded attach capability", req.Tool)
	}
	if capab.Attach.Subcommand != req.Subcommand {
		return nil, fmt.Errorf("attach: subcommand %q does not match tool %q's grounded attach subcommand %q",
	...
	// On a successful AUTO-resume spawn, supersede the old rediscovered orphan
	// row (end_reason='resumed') so it can't be re-offered across a later restart
	// (H2). Stamp by the exact PREDECESSOR run id resolved from the rediscovery
	// map — NOT by session — so the fresh run just spawned here (which correlates
	// to the same session via OOB immediately for codex, before this stamp could
	// otherwise run) is never mislabeled 'resumed', which would block its shutdown
	// sweep + its own restart offer (finding: wrong-run supersede). Best-effort; a
	// failure only risks re-offering a session whose new run is already live (the
	// hub guard + flock catch that anyway). Skipped when the predecessor can't be
	// resolved (nothing rediscovered to supersede).
	if req.AutoResume && req.ResumeSession != "" && h.supersedeResumed != nil && h.resumePredecessors != nil {
		if predRunIDs, ok := h.resumePredecessors(req.ResumeSession); ok && len(predRunIDs) > 0 {
			// Stamp ALL of this session's startup-eligible predecessor orphans,
			// not just the newest — older historical duplicates and prior
			// stamp-failure rows would otherwise stay offerable on every future
			// restart (round-4 multi-orphan finding). The fresh run just spawned
			// here was minted THIS process, so it cannot be in the startup
			// rediscovery set; guard against its id anyway so a resolver quirk can
			// never mislabel the live replacement 'resumed' (which would block its
			// own shutdown sweep + restart offer — the wrong-run-supersede class).
	...
	return &attachSession{
		...
		output: sub,
		corr:   corr,
		// Translate the termsession lease-revocation error into the attachsock
		// sentinel so the server can relay a NON-FATAL writer_revoked notice
		// (A5). attachsock never imports termsession — the mapping lives here,
		// at the boundary.
		write:  func(p []byte) (int, error) { return writeMappingRevoked(lease.Write, p) },
		resize: lease.Resize,
...
// writer lease. Its fields are plain funcs/readers so the type is testable in
// isolation with stubs (the deliverable-(a) unit test).
type attachSession struct {
	...
	exit    func() (int, bool)
	release func() // release the writer lease + unsubscribe the viewer
...
// Write forwards raw bytes (the client's keystrokes) to the PTY.
func (s *attachSession) Write(p []byte) (int, error) { return s.write(p) }

// Resize resizes the PTY.
func (s *attachSession) Resize(rows, cols uint16) error { return s.resize(rows, cols) }
