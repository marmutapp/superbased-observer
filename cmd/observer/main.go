...
//
// recipeName, when non-empty, names an embedded [config.LoadRecipe]
// recipe applied between Default and the on-disk config files. Used
// by `observer start --recipe <name>`; other callers pass "".
func buildWatcher(ctx context.Context, configPath, recipeName string) (*watcher.Watcher, func(), error) {
	return buildWatcherWithOverride(ctx, configPath, recipeName, "")
}
...
func buildWatcherWithOverride(ctx context.Context, configPath, recipeName, adapterFilter string) (*watcher.Watcher, func(), error) {
