...
func wireAIClients(opts WireAIClientsOptions) (lines []string, claudeProxyHint, codexProxyHint string, codexHooksHint bool, err error) {
	...
	if !opts.SkipExtension && anyExtensionSupported() {
		runBrowserExtensionStep(&buf, opts.HomeDir, opts.DryRun)
	}
...
// runBrowserExtensionStep writes the native-messaging host manifest for
// every detected Chromium-family browser (the 4th consent step's write). It
// is the browser rail's peer of hook.Registry.Register: detection is "a
// browser profile dir exists"; a machine with no Chromium browser writes
// nothing. Honest output: the manifest carries a PLACEHOLDER extension id
// until the operator installs the published extension and fills in its id.
// Returns human-readable summary lines. dryRun previews without writing.
func runBrowserExtensionStep(buf *strings.Builder, homeDir string, dryRun bool) {
	reg, err := browserhost.NewRegistrar(browserhost.Options{Home: homeDir, DryRun: dryRun})
	if err != nil {
	...
	det := reg.Detect()
	if len(det) == 0 {
		return // no Chromium browser — nothing to wire, silently.
	}
	for _, res := range reg.Register() {
		...
		case res.AlreadySet:
			fmt.Fprintf(buf, "browser (%s): native-messaging host already set in %s\n", res.Browser, res.ConfigPath)
		case res.Wrote && dryRun:
			fmt.Fprintf(buf, "browser (%s): would write native-messaging host → %s\n", res.Browser, res.ConfigPath)
		case res.Wrote:
			fmt.Fprintf(buf, "browser (%s): wrote native-messaging host → %s\n", res.Browser, res.ConfigPath)
		}
	}
	if !dryRun {
		fmt.Fprintln(buf, "browser: after installing the extension, set its id in the host manifest allowed_origins (currently a placeholder).")
	}
}
