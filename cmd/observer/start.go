	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !noDashboard {
				...
				// PTY launcher exist). Same assembly as `observer dashboard`.
				...
				opts := dashboard.Options{
					...
					// graceful shutdown (component drain + launchClose PTY reap
			...
			g.Go(func() error {
				...
				recv, err := browseringest.New(browseringest.Options{
					...
					Handler: func(ctx context.Context, body []byte) error {
						return ingestBrowserTurn(ctx, bst, body, browserCfg)
					},
				})
				if err != nil {
