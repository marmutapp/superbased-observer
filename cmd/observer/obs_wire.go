...
func obsBuildJudge(cfg config.Config) eval.JudgeClient {
	...
	return obsJudgeClient{
		client:       secureJudgeEgress(newChatCompletionsJudge(baseURL, os.Getenv(keyEnv)), judgeHostingLabel(baseURL, jc.Model)),
...
func obsBuildAdmissionJudge(cfg config.Config) (admission.JudgeClient, string, time.Duration) {
	...
	client := secureJudgeEgress(newChatCompletionsJudge(baseURL, os.Getenv(keyEnv)), hosting)
